...
/**
 * 大天使任务脚本（任务 ID 20112）。
 * Archdaeva quest script (quest ID 20112).
 *
 * @author (Encom)
 */
public class _20112 extends QuestHandler
{
	...
	private final static int[] Ab1EreshFiEvent = {885140}; //??  .
    ...
    public void register() {
        ...
        } for (int mob: Ab1EreshFiEvent) { //??  .
		    qe.registerQuestNpc(mob).addOnKillEvent(questId);
		}
		qe.registerOnLevelUp(questId);
		qe.registerQuestItem(182216238, questId); // ? ??.
		qe.registerOnEnterZoneMissionEnd(questId);
		qe.registerQuestNpc(885141).addOnKillEvent(questId); //??  ?.
    }
            ...
            if (targetId == 805356) { //Pontekane.
                ...
                switch (env.getDialog()) {
				    case START_DIALOG: {
						...
						return defaultCloseDialog(env, 1, 2, false, false, 182216238, 1, 0, 0); // ? ??.
