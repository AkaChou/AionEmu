...
/**
 * 伊卢玛任务脚本：Mystery Of The Aetherion（任务 ID 15605）。
 * Iluma quest script: Mystery Of The Aetherion (quest ID 15605).
 *
			...
			if (targetId == 806162) { //Polonius.
				switch (env.getDialog()) {
                    case START_DIALOG: {
						...
						removeQuestItem(env, 182216010, 1); //? ? ? ?.
						removeQuestItem(env, 182216011, 1); // ? ? ?.
						return closeDialogWindow(env);
