...
/**
 * 诺斯沃尔德任务脚本：The White Sinsye（任务 ID 25606）。
 * Norsvold quest script: The White Sinsye (quest ID 25606).
 *
...
public class _25606The_White_Sinsye extends QuestHandler {
    ...
    public void register() {
		...
		qe.registerQuestNpc(806178).addOnTalkEvent(questId); // ?.
		qe.registerQuestNpc(806156).addOnTalkEvent(questId); //? ? ?.
		qe.registerQuestNpc(806157).addOnTalkEvent(questId); //.
		qe.registerQuestNpc(703140).addOnTalkEvent(questId); //?  ??.
		...
		if (env.getVisibleObject() instanceof Npc) {
            ...
            if (targetId == 806175) { //Chaelsean.
                if (env.getDialog() == QuestDialog.START_DIALOG) {
					removeQuestItem(env, 182216006, 1); //? ?.
            ...
            if (var == 5) {
				switch (targetId) {
                    case 241220: { //White Sinsye.
						qs.setQuestVar(6);
					    updateQuestStatus(env);
						QuestService.addNewSpawn(220110000, 1, 806157, npc.getX(), npc.getY(), npc.getZ(), (byte) 0); //.
						return true;
					}
                }
			}
