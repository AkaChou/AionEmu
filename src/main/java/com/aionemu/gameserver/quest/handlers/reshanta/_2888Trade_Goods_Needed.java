...
/**
 * 雷山塔任务脚本：Trade Goods Needed（任务 ID 2888）。
 * Reshanta quest script: Trade Goods Needed (quest ID 2888).
 *
...
public class _2888Trade_Goods_Needed extends QuestHandler {
	...
	public boolean onDialogEvent(QuestEnv env) {
		...
		int var = qs.getQuestVarById(0);
            ...
            else if (targetId == 278151) { //Ickulnerk.
				switch (env.getDialog()) {
					case START_DIALOG: {
						if (var == 1) {
            ...
            else if (targetId == 278087) { //Gagrad.
				switch (env.getDialog()) {
					case START_DIALOG: {
						if (var == 4) {
