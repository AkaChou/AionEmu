...
/**
 * 潘德莫尼姆任务脚本（任务 ID 29900）。
 * Pandaemonium quest script (quest ID 29900).
 *
...
public class _29900 extends QuestHandler {
	...
	public boolean onLvlUpEvent(QuestEnv env) {
		...
		if (player.getLevel() >= 10 && (qs == null || qs.getStatus() == QuestStatus.NONE) && player.getRace() == Race.ASMODIANS) {
	...
	public boolean onEnterWorldEvent(QuestEnv env) { // Fix for player who already have this Quest
		Player player = env.getPlayer();
		QuestState qs = player.getQuestStateList().getQuestState(questId);
		if (qs != null && qs.getStatus() == QuestStatus.START && player.getInventory().getItemCountByItemId(190080011) < 1 && player.getWorldId() == 120010000) {
			return giveQuestItem(env, 190080011, 1);
		}
		return false;
	}
