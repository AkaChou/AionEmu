...
/**
 * 诺斯沃尔德任务脚本：An Adventure In Time（任务 ID 25605）。
 * Norsvold quest script: An Adventure In Time (quest ID 25605).
 *
...
public class _25605An_Adventure_In_Time extends QuestHandler {
    ...
    public void register() {
		...
		qe.registerQuestNpc(806199).addOnTalkEvent(questId); //?? ? ?.
			...
			if (targetId == 806174) { //Chardelil.
				...
				switch (env.getDialog()) {
					case START_DIALOG: {
						...
						removeQuestItem(env, 182216005, 1); //?  .
						return closeDialogWindow(env);
					}
				}
			}
