...
/**
 * 制作任务脚本：Master Armorsmiths Potential（任务 ID 19014）。
 * Crafting quest script: Master Armorsmiths Potential (quest ID 19014).
 *
 * @author Thuatan
 */
			...
			switch (targetId) {
				case 203791:
					switch (env.getDialog()) {
						...
						case STEP_TO_10:
							giveQuestItem(env, 152201806, 1);
							qs.setQuestVarById(0, 1);
							updateQuestStatus(env);
					        return closeDialogWindow(env);
						case STEP_TO_20:
							giveQuestItem(env, 152201807, 1);
							qs.setQuestVarById(0, 1);
							updateQuestStatus(env);
					        return closeDialogWindow(env);
					}
