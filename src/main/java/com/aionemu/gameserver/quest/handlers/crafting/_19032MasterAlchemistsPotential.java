...
/**
 * 制作任务脚本：Master Alchemists Potential（任务 ID 19032）。
 * Crafting quest script: Master Alchemists Potential (quest ID 19032).
 *
 * @author Thuatan
 */
			...
			switch (targetId) {
				case 203787:
					switch (env.getDialog()) {
						...
						case STEP_TO_10:
							giveQuestItem(env, 152202147, 1);
							giveQuestItem(env, 152020248, 1);
							qs.setQuestVarById(0, 1);
							updateQuestStatus(env);
					        return closeDialogWindow(env);
						case STEP_TO_20:
							giveQuestItem(env, 152202148, 1);
							giveQuestItem(env, 152020248, 1);
							qs.setQuestVarById(0, 1);
							updateQuestStatus(env);
					        return closeDialogWindow(env);
