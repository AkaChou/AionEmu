...
/**
 * 污名幻象任务脚本：A Sliver Of Darkness（任务 ID 1929）。
 * Stigma Vision quest script: A Sliver Of Darkness (quest ID 1929).
 */
public class _1929A_Sliver_Of_Darkness extends QuestHandler {
    ...
    public boolean onLvlUpEvent(QuestEnv env) {
        Player player = env.getPlayer();
        boolean started = defaultOnLvlUpEvent(env);
        QuestState qs = player.getQuestStateList().getQuestState(questId);
        if (qs != null && qs.getStatus() == QuestStatus.START) {
            if (player.havePermission(MembershipConfig.STIGMA_SLOT_QUEST)) {
                completeQuest(player, questId);
                player.getCommonData().addExp(457760, null);
            }
        }
        return true;
    }
    ...
    private static void completeQuest(Player player, int questId) {
        QuestState qs = player.getQuestStateList().getQuestState(questId);
        if (qs == null) {
            qs = new QuestState(questId, QuestStatus.COMPLETE, 0, 1, null, 0, null);
