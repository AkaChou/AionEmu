...
/**
 * 制作任务脚本：Master Weaponsmiths Potential（任务 ID 29008）。
 * Crafting quest script: Master Weaponsmiths Potential (quest ID 29008).
 *
 * @author Thuatan
 */
			...
			switch (targetId) {
				case 204105:
					switch (env.getDialog()) {
						...
						case STEP_TO_10:
							giveQuestItem(env, 152206708, 1);
							giveQuestItem(env, 152029251, 1);
							qs.setQuestVarById(0, 1);
							updateQuestStatus(env);
					        return closeDialogWindow(env);
						case STEP_TO_20:
							giveQuestItem(env, 152206709, 1);
							giveQuestItem(env, 152029251, 1);
							qs.setQuestVarById(0, 1);
							updateQuestStatus(env);
					        return closeDialogWindow(env);
