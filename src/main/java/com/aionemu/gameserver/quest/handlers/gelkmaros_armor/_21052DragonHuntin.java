...
/**
 * 格尔克马洛斯防具任务脚本：Dragon Huntin（任务 ID 21052）。
 * Gelkmaros armor quest script: Dragon Huntin (quest ID 21052).
 */
		...
		if (qs == null || qs.getStatus() == QuestStatus.NONE || qs.canRepeat()) {
			...
			if (targetId == 799268) { //Nysdvor
				if (dialog == QuestDialog.START_DIALOG) {
					...
					long itemCount = player.getInventory().getItemCountByItemId(182207840);
					if (player.getInventory().tryDecreaseKinah(50000) && itemCount > 29) {
						player.getInventory().decreaseByItemId(182207840, 30);
