...
public class IronWallWarfrontInstance extends GeneralInstanceHandler {
	...
	private static final String PHASE_ADJUSTING = "ADJUSTING";
	...
	public void onInstanceCreate(WorldMapInstance instance) {
		...
		adjustmentMillis = battleground.requiredInt("adjust_time") * 1_000L;
		preparationMillis = battleground.requiredInt("wait_time") * 1_000L;
		battleMillis = battleground.requiredInt("limit_time") * 1_000L;
		adjustmentMessageId = requiredMessage(battleground, "adjust_time_start_msg");
		preparationMessageId = requiredMessage(battleground, "wait_time_start_msg");
		...
		reward = new IronWallWarfrontReward(mapId, instanceId);
		restoreReward(battleground.requiredInt("base_score"));
		adjustmentStartedAt = runtimeState().getLong(STATE_PREFIX + "adjustment.started", 0);
	...
	private Row battleground() {
		...
		return rows.stream().filter(row -> row.intValue("spawn_page", 0) == spawnPage).findFirst()
			.orElseGet(() -> rows.size() == 1 ? rows.getFirst() : missingBattleground(spawnPage));
	}

	private Row missingBattleground(int spawnPage) {
		throw new IllegalStateException("Missing Iron Wall battleground data for spawn page " + spawnPage);
	}

	private int requiredMessage(Row row, String attribute) {
		String name = row.value(attribute);
		Integer messageId = name == null ? null : DataManager.RETAIL_AI_DATA.findStringId(name);
		if (messageId == null) {
			throw new IllegalStateException("Missing Iron Wall battleground message " + name);
		}
		return messageId;
	}

	private void restorePhase() {
		String phase = runtimeState().get(STATE_PREFIX + "phase", PHASE_ADJUSTING);
		switch (phase) {
			case PHASE_FINISHED -> {
			...
			case PHASE_ADJUSTING -> {
				reward.setInstanceScoreType(InstanceScoreType.PREPARING);
				if (adjustmentStartedAt > 0) {
					scheduleDeadline("adjustment",
						restoreDeadline("adjustment", adjustmentStartedAt, adjustmentMillis), this::startPreparation);
				}
			}
			default -> throw new IllegalStateException("Unknown Iron Wall phase " + phase);
		}
	}
	...
	public void onEnterInstance(Player player) {
		IronWallWarfrontPlayerReward playerReward = registerPlayer(player);
	...
	private synchronized void startAdjustment() {
		if (adjustmentStartedAt != 0 || destroyed || reward.isRewarded()) {
			return;
		}
		adjustmentStartedAt = System.currentTimeMillis();
		long deadline = adjustmentStartedAt + adjustmentMillis;
		runtimeState().put(STATE_PREFIX + "adjustment.started", adjustmentStartedAt);
		runtimeState().put(STATE_PREFIX + "adjustment.deadline", deadline);
		runtimeState().put(STATE_PREFIX + "phase", PHASE_ADJUSTING);
		sendMsgByRace(adjustmentMessageId, Race.PC_ALL);
		scheduleDeadline("adjustment", deadline, this::startPreparation);
	}
	...
	private synchronized void startPreparation() {
		...
		preparationStartedAt = Math.min(System.currentTimeMillis(),
			runtimeState().getLong(STATE_PREFIX + "adjustment.deadline", System.currentTimeMillis()));
		...
		sendMsgByRace(preparationMessageId, Race.PC_ALL);
	...
	private void settlePlayers(long endedAt) {
		...
		for (IronWallWarfrontPlayerReward playerReward : List.copyOf(reward.getInstanceRewards())) {
	...
	private int getTime() {
		...
		String key = switch (phase) {
			case PHASE_ADJUSTING -> "adjustment.deadline";
			case PHASE_PREPARING -> "preparation.deadline";
			case PHASE_BATTLE -> "battle.deadline";
			default -> null;
		};
		return key == null ? 0 : (int) Math.max(0, runtimeState().getLong(STATE_PREFIX + key, now) - now);
	}
	...
	public boolean onDie(Player player, Creature lastAttacker) {
		IronWallWarfrontPlayerReward playerReward = reward.getPlayerReward(player.getObjectId());
	...
	private void updateScore(Player player, Creature target, int points, boolean pvpKill) {
		...
		for (Player recipient : recipients) {
			IronWallWarfrontPlayerReward recipientReward = registerPlayer(recipient);
		...
		if (pvpKill && points > 0) {
			...
			IronWallWarfrontPlayerReward killerReward = registerPlayer(player);
	...
	public boolean supportsRetailNpcScore(int npcId, int scoreApplyType) {
		...
		return score != null && score.scoreApplyType() == scoreApplyType && scoreApplyType >= 0 && scoreApplyType <= 3
	...
	public boolean onRetailNpcScore(Player player, Npc npc, int scoreApplyType, int points) {
		...
		consumeNpcScore(player, npc, scoreApplyType, points);
		return true;
	}
	...
	public void onDie(Npc npc) {
		...
		if (score == null || score.equalizingScore() != 0 || score.scoreApplyType() < 0 || score.scoreApplyType() > 3) {
			return;
		}
		consumeNpcScore(npc.getAggroList().getMostPlayerDamage(), npc, score.scoreApplyType(), score.value());
	}

	private synchronized boolean consumeNpcScore(Player player, Npc npc, int scoreApplyType, int points) {
		if (!reward.isStartProgress() || points == 0 || requiresScorePlayer(scoreApplyType) && player == null) {
			return false;
		}
		...
		switch (scoreApplyType) {
			...
			case 1 -> addTeamScore(Race.ELYOS, points);
			case 2 -> addTeamScore(Race.ASMODIANS, points);
			case 3 -> {
				addTeamScore(Race.ELYOS, points);
				addTeamScore(Race.ASMODIANS, points);
			}
			default -> throw new IllegalArgumentException("Unsupported retail score apply type " + scoreApplyType);
		}
		if (scoreApplyType != 0) {
			broadcastScoreTables();
		}
	...
	static boolean requiresScorePlayer(int scoreApplyType) {
		return scoreApplyType == 0;
	}

	private IronWallWarfrontPlayerReward registerPlayer(Player player) {
		IronWallWarfrontPlayerReward playerReward = reward.getPlayerReward(player.getObjectId());
		if (playerReward == null) {
			playerReward = new IronWallWarfrontPlayerReward(player.getObjectId(), reward.getBuffId(), player.getRace());
	...
	private void restoreReward(int baseScore) {
		...
		for (String value : players.split(",")) {
			...
			IronWallWarfrontPlayerReward playerReward = new IronWallWarfrontPlayerReward(playerId, reward.getBuffId(), race,
	...
	private void persistPlayer(IronWallWarfrontPlayerReward playerReward) {
	...
	private void openFirstDoors() {
		for (int doorId : new int[] { 2, 17, 26, 35 }) {
