...
public class Event_ContaminatedUnderpathInstance extends GeneralInstanceHandler {
	...
	public void onOpenDoor(Player player, int doorId) {
		if (doorId != 57 || runtimeState().getBoolean(STATE + "completed", false)) {
			return;
		}
		setDoorState(57, true);
	...
	public void doReward(Player player) {
		...
		IDEventDefPlayerReward reward = getOrCreatePlayerReward(player);
		if (reward.isRewarded()) {
			return;
		}
		RewardPlan plan = InstanceSettlementService.timeAttackPlan(mapId, instanceReward.getRank());
		reward.setScoreAP(plan.ap());
		reward.setWrapCashIDEventDefLiveSRank(Math.toIntExact(plan.itemCount(188058265)));
		reward.setWrapCashIDEventDefLiveARank(Math.toIntExact(plan.itemCount(188058266)));
		reward.setWrapCashIDEventDefLiveBRank(Math.toIntExact(plan.itemCount(188058267)));
	...
	private synchronized void startMainTimer(long startAt) {
		...
		RetailConditionSpawnEngine.setVariable(instance, "TIMEATTACK_PLAY_START", 1, 0);
	...
	private IDEventDefPlayerReward getOrCreatePlayerReward(Player player) {
	...
	private void applyEntryEffect(Player player) {
		int skillId = player.getRace() == Race.ASMODIANS ? 4940 : 4935;
