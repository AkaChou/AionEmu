...
/**
 * 管理员属性覆盖与羽饰强化调试命令。
 * Admin command for stat overrides and plume enchant debug.
 * <p>
...
public class Stats extends AdminCommand implements StatOwner {
	...
	/**
	 * 属性基值覆盖函数（支持多种 StatEnum）。
	 * Base-stat override function for many StatEnum values.
	 */
		...
		public void apply(com.aionemu.gameserver.model.stats.calc.Stat2 stat)
		{
			switch (this.stat)
			{
				...
				case POWER :
					...
					stat.setBase(modifierPower);
					break;
				case MAXHP:
					float modifierHp = (float) modifier;
					stat.setBase(modifierHp);
					break;
				case MAXMP:
					float modifierMp = (float) modifier;
					stat.setBase(modifierMp);
					break;
				case REGEN_HP:
				...
				case MAGIC_SKILL_BOOST_RESIST:
					...
					stat.setBase(modifierMSBResist);
					break;
				default:
					break;
			}
