...
public class MySQL8WeddingDAO extends WeddingDAO {
	...
	private static final String DELETE_QUERY = "DELETE FROM `weddings` WHERE (`player1`=? AND `player2`=?) OR (`player2`=? AND `player1`=?)";
	...
	public int loadPartnerId(final Player player) {
		...
		try (Connection con = DatabaseFactory.getConnection();
			...
			log.error("Could not get partner for player: " + playerId + " from DB", e);
		}
		return partnerId;
	}
