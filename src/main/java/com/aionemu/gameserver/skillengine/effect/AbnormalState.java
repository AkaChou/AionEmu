...
public enum AbnormalState {
	...
	CANT_ATTACK_STATE(SPIN.id | SLEEP.id | STUN.id | STUMBLE.id | STAGGER.id | OPENAERIAL.id | PARALYZE.id | FEAR.id
			| CANNOT_MOVE.id | CONFUSE.id),
	/** 无法移动的状态组合 / States that prevent movement */
	CANT_MOVE_STATE(SPIN.id | ROOT.id | SLEEP.id | STUMBLE.id | STUN.id | STAGGER.id | OPENAERIAL.id | PARALYZE.id
			| CANNOT_MOVE.id),
	/** 强制下坐骑的状态组合 / States that force dismount */
	DISMOUT_RIDE(SPIN.id | ROOT.id | SLEEP.id | STUMBLE.id | STUN.id | STAGGER.id | OPENAERIAL.id | PARALYZE.id
