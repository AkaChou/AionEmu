...
public class MaterialZoneTemplate extends ZoneTemplate {
	...
	public MaterialZoneTemplate(Spatial geometry, int mapId) {
		...
		if (geometry.getName().indexOf("CYLINDER") != -1 || geometry.getName().indexOf("CONE") != -1
			...
			cylinder = new Cylinder(center.x, center.y, Math.max(box.getXExtent(), box.getYExtent() + 1),
					center.z + box.getZExtent() + 1, center.z - box.getZExtent() - 1);
		} else if (geometry.getName().indexOf("SEMISPHERE") != -1) {
			areaType = AreaType.SEMISPHERE;
			semisphere = new Semisphere(center.x, center.y, center.z,
					Math.max(Math.max(box.getXExtent(), box.getYExtent()), box.getZExtent()) + 1);
		} else {
			areaType = AreaType.SPHERE;
			sphere = new Sphere(center.x, center.y, center.z,
					Math.max(Math.max(box.getXExtent(), box.getYExtent()), box.getZExtent()) + 1);
		}
	}
}
