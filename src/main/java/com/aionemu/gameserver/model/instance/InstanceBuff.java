...
import com.aionemu.gameserver.dataholders.RetailInstanceData.BonusAttribute;
...
public class InstanceBuff implements StatOwner {
	...
	private final List<BonusAttribute> bonusAttributes;
	private long startTime;

	public InstanceBuff(int buffId) {
		bonusAttributes = DataManager.RETAIL_INSTANCE_DATA.bonusAttributes(buffId);
	}

	/** 应用效果。 / Apply effect. */
	public void applyEffect(Player player, int time) {

		if (hasInstanceBuff() || bonusAttributes.isEmpty()) {
			return;
		}
		if (time != 0) {
		...
		for (BonusAttribute attribute : bonusAttributes) {
			StatEnum stat = attribute.stat();
			int statToModified = player.getGameStats().getStat(stat, 0).getBase();
			int value = attribute.value();
			int valueModified = attribute.func() == Func.PERCENT ? (statToModified * value / 100)
					: (value);
	...
	public void applyPledge(Player player, int buffId) {
		...
		for (BonusAttribute attribute : bonusAttributes) {
			if (attribute.func() == Func.PERCENT) {
				functions
						.add(new StatRateFunction(attribute.stat(), attribute.value(), true));
			} else {
				functions.add(new StatAddFunction(attribute.stat(), attribute.value(), true));
			}
		}
