...
public class OptimisticLinkedQueue<E> extends AbstractQueue<E> implements Queue<E>, java.io.Serializable {
	...
	/**
	 * Pointer to the head node, initialized to a dummy node. The first actual node
	 * is at head.getPrev().
	 */
	...
	/**
	 * Dequeues an element from the queue. After a successful casHead, the prev and
	 * next pointers of the dequeued node are set to null to allow garbage
	 * collection.
	 */
	public E poll() {
		for (;;) {
