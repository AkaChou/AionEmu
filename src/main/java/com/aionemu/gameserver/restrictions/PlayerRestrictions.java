...
public class PlayerRestrictions extends AbstractRestrictions {
	...
	public boolean canAffectBySkill(Player player, VisibleObject target, Skill skill) {
		...
		if (skill.getSkillTemplate().hasResurrectEffect()
				&& (!(target instanceof Player) || !((Creature) target).getLifeStats().isAlreadyDead()
						|| (!((Creature) target).isInState(CreatureState.DEAD)
	...
	public boolean canUseSkill(Player player, Skill skill) {
		...
		if ((template.getType() == SkillType.PHYSICAL)
				&& (player.getEffectController().isAbnormalSet(AbnormalState.BIND))) {
	...
	public boolean canInviteToLeague(Player player, Player target) {
		...
		if (target.isInGroup2()) {
			...
			if (targetGroup.isLeader(target)) {
				PacketSendUtility.sendPacket(player,
						SM_SYSTEM_MESSAGE.STR_UNION_INVITE_HIM(target.getName(), targetGroup.getLeader().getName()));
				return false;
			}
	...
	public boolean canUseItem(Player player, Item item) {
		...
		if (item.getItemTemplate().isArchdaeva() && !player.getCommonData().isArchDaeva()) {
			PacketSendUtility.sendPacket(player,
					SM_SYSTEM_MESSAGE.STR_CANNOT_USE_ITEM_INVALID_HIGHDEVA(item.getName()));
			return false;
		}
