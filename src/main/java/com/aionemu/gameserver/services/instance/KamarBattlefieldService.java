...
public class KamarBattlefieldService {
	...
	public void initKamarBattlefield() {
		if (AutoGroupConfig.KAMAR_ENABLED) {
			 ...
			 * 执行任务。
			 * Runs the task.
			 */GameCronServices.cronService().schedule(() -> startKamarRegistration(), AutoGroupConfig.KAMAR_SCHEDULE_MIDNIGHT);
		}
	}
	...
	public static KamarBattlefieldService getInstance() {
		...
		KamarBattlefieldService provided = provider == null ? null : provider.getIfAvailable();
		if (provided == null) {
			throw new IllegalStateException("KamarBattlefieldService 未由 Spring 提供："
