...
* Initializes Suspicious Ophidan Bridge registration by scheduling open windows per config.
	...
	public void initSuspiciousOphidan() {
		if (AutoGroupConfig.OPHIDAN_WARPATH_ENABLED) {
			 ...
			 * 执行任务。
			 * Runs the task.
			 */GameCronServices.cronService().schedule(() -> startSuspiciousOphidanRegistration(), AutoGroupConfig.OPHIDAN_WARPATH_SCHEDULE_MIDNIGHT);
		}
	}
	...
	public static SuspiciousOphidanBridgeService getInstance() {
		...
		SuspiciousOphidanBridgeService provided = provider == null ? null : provider.getIfAvailable();
		if (provided == null) {
			throw new IllegalStateException("SuspiciousOphidanBridgeService 未由 Spring 提供："
