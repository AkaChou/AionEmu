...
* Initializes Engulfed Ophidan Bridge registration by scheduling open windows per config.
	...
	public static EngulfedOphidanBridgeService getInstance() {
		...
		EngulfedOphidanBridgeService provided = provider == null ? null : provider.getIfAvailable();
		if (provided == null) {
			throw new IllegalStateException("EngulfedOphidanBridgeService 未由 Spring 提供："
