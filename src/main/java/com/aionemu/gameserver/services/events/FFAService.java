...
public class FFAService {
	...
	public void onDie(final Player player, Creature lastAttacker) {
		...
		player.getEffectController().removeEffectByDispelCat(DispelCategoryType.ALL, SkillTargetSlot.DEBUFF, 100, 2, 100);
	...
	public static final FFAService getInstance() {
		...
		FFAService provided = provider == null ? null : provider.getIfAvailable();
		if (provided == null) {
			throw new IllegalStateException("FFAService 未由 Spring 提供："
