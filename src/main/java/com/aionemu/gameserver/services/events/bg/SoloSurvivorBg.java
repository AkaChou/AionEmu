...
public class SoloSurvivorBg extends Battleground {
	...
	/** 是否已触发结束。 / Whether end already called. */
	private boolean endCalled = false;

	/**
	 * 初始化 1v1 生存战名称、规模与地图出生点。
	 * Initializes solo survivor name, sizes and map spawn points.
	 */
	public SoloSurvivorBg() {
		super.name = "[1 VS 1]";
		super.description = "You must kill your opponent to win. You have " + maxRounds
				+ " round. The player who gets the most kill's, win the match !!!";
		super.minSize = 2;
		super.maxSize = 4;
		super.teamCount = 1;
		...
		// 痛苦龙主避难所。 / Anguished Dragon Lord's Refuge.
		...
		// 术古皇帝宝库。 / The Shugo Emperor's Vault.
		...
		// 秘密军需工厂。 / Secret Munitions Factory.
		...
		// 圣所地下竞技场。 / Sanctum Underground Arena.
		...
		// 阿尔奎米亚研究中心。 / Alquimia Research Center.
		...
		// 现场派对音乐厅。 / Live Party Concert Hall.
	...
	/**
	 * 开始生存战对局。
	 * Starts the solo survivor match.
	 */
	...
	public boolean createTournament(List<List<Player>> players) {
		...
		this.description = "You must kill your opponent to win. You have " + maxRounds
				+ " round. The player who gets the most kill's, win the match !!!";
		startMatch();
		return true;
	}
	...
	/**
	 * 开始新回合：重置状态并重新布置。
	 * Starts a new round: reset state and re-position players.
	 */
		...
		if (winner == null) {
			...
			for (Player pl : super.getPlayers()) {
				if (pl.getObjectId() == winner.getObjectId()) {
					...
					super.scheduleAnnouncement(pl, "You have received rewards " + (is1v1() ? "for your efforts!" : "."),
							3000);
				} else {
