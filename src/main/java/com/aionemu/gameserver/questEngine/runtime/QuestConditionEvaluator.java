		...
		for (QuestCondition condition : conditions) {
			boolean matched = switch (condition) {
				...
				case QuestCondition.QuestVariableIs variable ->
					variables.getOrDefault(variable.field(), Integer.MIN_VALUE)
						== variable.value();
				...
				case QuestCondition.VariableSumIs variable ->
					variableSum(variables, variable.fields()) == variable.value();
				...
				case QuestCondition.AdvancedClassIs playerClass -> advancedClass(snapshot, playerClass.playerClass());
