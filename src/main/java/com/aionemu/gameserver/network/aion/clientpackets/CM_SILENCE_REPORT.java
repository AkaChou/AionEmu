...
public class CM_SILENCE_REPORT extends AionClientPacket {
	...
	protected void runImpl() {
		...
		if (targetPlayer == null || targetPlayer.isGM()) {
			...
			PacketSendUtility.sendPacket(activePlayer,
					new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_MSG_REPORT_CHAT_CANT_NO_USER, 0, 0, 0));
			return;
		} else if (activePlayer.getLevel() < 10) {
			PacketSendUtility.sendPacket(activePlayer,
					new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_MSG_REPORT_CHAT_CANT_LOW_LEVEL, 0, 0, 0));
			return;
		} else if (activePlayer.getCommonData().getRace() != targetPlayer.getCommonData().getRace()) {
			PacketSendUtility.sendPacket(activePlayer,
					new SM_QUESTION_WINDOW(SM_QUESTION_WINDOW.STR_MSG_REPORT_CHAT_CANT_OTHER_RACE, 0, 0, 0));
			return;
		} else {
			report++;
			...
			PacketSendUtility.sendPacket(activePlayer, new SM_QUESTION_WINDOW(
					SM_QUESTION_WINDOW.STR_MSG_REPORT_CHAT_CONFIRM, 0, 0, targetPlayer.getName()));
			PacketSendUtility.sendMessage(activePlayer,
					"Silence report to " + targetPlayer.getName() + " count " + report);
			if (targetPlayer.getSilenceReportCount() >= 2) { // TODO: dont know in Official set how much count of
																// reporter for started silencing.
				targetPlayer.setGagged(true);
				PacketSendUtility.sendMessage(targetPlayer,
						"You got silence punish, because some players were reported you spamming.");
				...
				targetPlayer.getController().addTask(TaskId.GAG,
						GameThreadPoolServices.threadPoolManager().schedule(new Runnable() {
							...
							public void run() {
								...
								PacketSendUtility.sendMessage(targetPlayer,
										"Silence punish is no longer apllied on you.");
							}
