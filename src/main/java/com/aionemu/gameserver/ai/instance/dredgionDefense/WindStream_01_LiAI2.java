...
/**
 * Dredgion Defense 副本 NPC AI：Wind Stream 01 Li（@AIName "WindStream_01_Li"），继承 NpcAI2。
 * Dredgion Defense instance NPC AI: Wind Stream 01 Li (@AIName "WindStream_01_Li"), extends NpcAI2.
 * @author Encom
 */
...
public class WindStream_01_LiAI2 extends NpcAI2
{
    ...
    private void windStreamAnnounce(final Npc npc, final int state) {
        ...
        npc.getPosition().getWorld().doOnAllPlayers(player -> PacketSendUtility.sendPacket(player, new SM_WINDSTREAM_ANNOUNCE(1, 302200000, 360, state)));
    }
