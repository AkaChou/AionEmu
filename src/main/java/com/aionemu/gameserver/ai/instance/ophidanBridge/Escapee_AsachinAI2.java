	...
	protected void handleCreatureMoved(Creature creature) {
		...
		if (getNpcId() == 235782 && isInRange(creature, 15) && creature instanceof Player) {
			if (startedEvent.compareAndSet(false, true)) {
				...
				// 逃犯阿萨钦正逃往哨所。跟随继续追击。 / Fugitive Asachin is attempting to flee to the next sentry post. Pursue the fugitive.
			...
			if (startedEvent.compareAndSet(false, true)) {
				switch (Rnd.get(1, 2)) {
					case 1:
						...
						// 逃犯阿萨钦正逃往两处哨所之一。跟随逃犯继续追击。 / Fugitive Asachin is attempting to flee to one of the two sentry posts. Follow the fugitive to continue the pursuit.
			...
			if (startedEvent.compareAndSet(false, true)) {
				...
				// 逃犯阿萨钦正逃往哨所。跟随继续追击。 / Fugitive Asachin is attempting to flee using teleport. Pursue the fugitive.
