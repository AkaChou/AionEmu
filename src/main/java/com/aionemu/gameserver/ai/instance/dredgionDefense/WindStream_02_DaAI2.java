...
/**
 * Dredgion Defense 副本 NPC AI：Wind Stream 02 Da（@AIName "WindStream_02_Da"），继承 NpcAI2。
 * Dredgion Defense instance NPC AI: Wind Stream 02 Da (@AIName "WindStream_02_Da"), extends NpcAI2.
 *
 * @author Encom
 */
...
public class WindStream_02_DaAI2 extends NpcAI2
{
    ...
    private void windStreamAnnounce(final Npc npc, final int state) {
        ...
        npc.getPosition().getWorld().doOnAllPlayers(player -> PacketSendUtility.sendPacket(player, new SM_WINDSTREAM_ANNOUNCE(1, 302300000, 368, state)));
    }
