...
/**
 * 定时服务的生命周期封装：幂等启动并记录加载耗时与失败原因。
 * Lifecycle wrapper for scheduled services: idempotent start with load timing and failure capture.
 */
@Component
...
public class GameScheduledServicesLifecycle {
    ...
    /**
     * 幂等启动定时服务；已加载则直接返回。
     * Idempotently start scheduled services; no-op when already loaded.
     */
