...
public class Base64 {
	 ...
	 * @return BASE64 编码后的字符数组，永不为 null
	...
	public static char[] encodeToChar(byte[] sArr, boolean lineSep) {
		...
		for (int s = 0, d = 0, cc = 0; s < eLen;) {
			// 将三个字节复制到 int 的低24位 Copy next three bytes into lower 24 bits of int
			int i = (sArr[s++] & 0xff) << 16 | (sArr[s++] & 0xff) << 8 | (sArr[s++] & 0xff);
			
			// 将 int 编码为四个字符 Encode the int into four chars
			dArr[d++] = CA[(i >>> 18) & 0x3f];
			dArr[d++] = CA[(i >>> 12) & 0x3f];
			dArr[d++] = CA[(i >>> 6) & 0x3f];
			dArr[d++] = CA[i & 0x3f];
	...
	public static byte[] decode(char[] sArr) {
		...
		// 检查合法字符（包括'='）是否能被4整除，符合 RFC 2045规范
		// 检查合法字符（含 =）是否按 RFC 2045 可被 4 整除。 / Check if legal chars (including '=') are evenly divisible by 4 as specified in RFC 2045
		if ((sLen - sepCnt) % 4 != 0) {
			return null;
		}
		...
		for (int s = 0, d = 0; d < len;) {
			// 将四个有效字符组装成一个 int Assemble three bytes into an int from four "valid" characters
			int i = 0;
			for (int j = 0; j < 4; j++) {
				int c = IA[sArr[s++]];
				if (c >= 0) {
	 ...
	 * @param lineSep 是否在76个字符后添加"\r\n"换行符
	 *               Optional "\r\n" after 76 characters, unless end of file.
	 * @return BASE64 编码后的字节数组，永不为 null
	 *         A BASE64 encoded array. Never null.
	 */
	public static byte[] encodeToByte(byte[] sArr, boolean lineSep) {
		...
		for (int s = 0, d = 0, cc = 0; s < eLen;) {
			...
			// 将 int 编码为四个 Base64 字符 Encode the int into four Base64 chars
	 ...
	 * @param lineSep 是否在76个字符后添加"\r\n"换行符
	 *               Optional "\r\n" after 76 characters, unless end of file.
	 * @return BASE64 编码后的字符串，永不为 null
	 *         A BASE64 encoded array. Never null.
	 */
