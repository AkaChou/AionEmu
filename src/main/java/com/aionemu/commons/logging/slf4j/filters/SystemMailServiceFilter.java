...
/**
 * 系统邮件服务日志过滤器
 * System mail service log filter that filters mail service related messages
 */
public class SystemMailServiceFilter extends Filter<ILoggingEvent> {
     ...
     * @return FilterReply.ACCEPT 如果消息以[SYSMAILSERVICE]开头；FilterReply.DENY 如果不是
     *         (FilterReply.ACCEPT if message starts with [SYSMAILSERVICE]; FilterReply.DENY if not)
     */
