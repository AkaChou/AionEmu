 ...
 * 随后在步骤 8 (s8) 通过 SELECT9 衔接对白，最终以 SELECT_QUEST_REWARD 播放影片 118 并进入 reward 结算。</p>
 */
class Quest10530ClientDialogAlignmentTest {
	private static final int JUCLEAS_NPC_ID = 203752;
	private static final int DYE_ITEM_ID = 182216164;
	private static final int FRAGMENT_ITEM_ID = 182216167;
	private static final int ALTAR_OBJECT_ID = 703387;
	...
	void step7CollectionHandoverAndStep8DialogContinuationContract() throws Exception {
		...
		// 2. 步骤 7: 尤克莱亚斯 QUEST_SELECT 必须在 s7 展示 SELECT8
		// Step 7: Jucleas QUEST_SELECT in s7 must show SELECT8
		QuestTransition select8 = talk(definition, "s7", "s7", JUCLEAS_NPC_ID, QuestDialogAction.QUEST_SELECT.id());
		...
		QuestTransition handOverSuccess = transition(definition, "s7", "s8",
			new QuestEvent.TalkToNpc(JUCLEAS_NPC_ID, QuestDialogAction.CHECK_USER_HAS_QUEST_ITEM.id()), 0);
		...
		QuestTransition questSelectInS8 = talk(definition, "s8", "s8", JUCLEAS_NPC_ID, QuestDialogAction.QUEST_SELECT.id());
		assertEquals(List.of(new AfterCommitAction.ShowQuestDialog(QuestDialogPage.SELECT9.id())),
			questSelectInS8.afterCommit());

		// 5. 步骤 8 完成奖励选择: SELECT_QUEST_REWARD 播放影片 118 并进入 reward
		// Step 8 reward selection: SELECT_QUEST_REWARD plays movie 118 and enters reward
		QuestTransition rewardTransition = talk(definition, "s8", "reward", JUCLEAS_NPC_ID,
			QuestDialogAction.SELECT_QUEST_REWARD.id());
		assertEquals(List.of(
			new AfterCommitAction.PlayMovie(118),
