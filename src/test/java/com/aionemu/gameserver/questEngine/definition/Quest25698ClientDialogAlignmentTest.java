...
/**
 * 验证任务 25698 的客户端报告/领奖对话只在完成五次击杀后开放，并保留满计数恢复路线。
 * Verifies quest 25698 opens the client report and reward dialogs only after five kills while keeping the
 * full-count recovery route.
 */
class Quest25698ClientDialogAlignmentTest {
	...
	void gatesTheClientReportPageOnCompletedKillProgress() {
		...
		assertEquals(Set.of(QuestDialogAction.QUEST_SELECT.id(),
			...
			"25698 only opens REWARD through the two client report dialogs");
		...
		assertEquals(KILLS_REQUIRED, definition.progressLayout().field("var1").maxValue(),
			"25698 kill counter ceiling");
		// 未满计数不可报告由 QuestPrematureRewardRouteExclusionTest 的 counter(25698, 5, ...) 逐计数证明。
		// Incomplete progress cannot report; proven per-count by the counter(25698, 5, ...) case there.
