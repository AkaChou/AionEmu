...
<quest-definition id="4011" version="1">
  ...
  <transitions>
    ...
    <transition source="unaccepted" target="started">
      <event>
        <dialog type="TALK_TO_NPC" npc-id="730139" action="SETPRO1"/>
      </event>
      <after-commit>
    ...
    <transition source="unaccepted" target="unaccepted">
      <event>
        <dialog type="TALK_TO_NPC" npc-id="730139" action="FINISH_DIALOG"/>
      </event>
      <after-commit>
        <close-dialog/>
      </after-commit>
    ...
    <transition source="started" target="step1">
      <event>
        <dialog type="TALK_TO_NPC" npc-id="205132" action="SETPRO2"/>
      </event>
      <after-commit>
        <sync-quest-state mode="LEVEL_AND_VISIBILITY_REFRESH"/>
        <close-dialog/>
    ...
    <transition source="step1" target="step2">
      <event>
        <dialog type="TALK_TO_NPC" npc-id="203522" action="SET_SUCCEED"/>
      </event>
      <after-commit>
        <sync-quest-state mode="LEVEL_AND_VISIBILITY_REFRESH"/>
        <close-dialog/>
    ...
    <dialog type="NPC_REPORT" npc-id="205132" source="step2" target="reward" page="DEFAULT_SUCCESS"/>
...
with open("src/main/resources/aion/data/static_data/quest_definition/quests/4011.xml", "w", encoding="utf-8") as f:
    f.write(content)
print("Updated 4011")
