...
- 无：所有有效查询都能在 Top-3 内找回对应 Pattern。
...
- AR-006、AR-008 共用 `.agents/summary/architecture-performance-refactor/2026-09-15-gameplay-jfr-hotspots.md`：排名 1
...
| P0–P2 改造后 JFR 量化记录 / JFR after the P0–P2 refactor。… | `AR-001` | 1 | `AR-001` |
| 启动加速：并行加载、刷怪并行与 Spring 锁消除 / Startup speedup: para… | `AR-001` | 1 | `AR-001` |
| 游戏内 JFR 热点分析 / Gameplay JFR hotspots。- 发起者 ： NpcMo… | `AR-006, AR-008` | 1 | `AR-006` |
| Spring 配置层解耦与碎片调度任务收拢记录（2026-09-18）。根据 Spring 迁移遗留… | `AR-011` | 1 | `AR-011` |
...
| 任务 2392 可选收集物清理与领奖卡死修复审计。在任务 1922 与 2947 的可选工作物品阻断… | `QE-007` | 1 | `QE-007` |
| 任务 19055 不可达接取前置审计。任务 19055 的 finished quest-id="1… | `QE-008` | 1 | `QE-008` |
