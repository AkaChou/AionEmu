...
# 批次 41 登记（2026-09-22）：潘盖亚要塞战三行族（14220 天族 / 24220 魔族）。
# 行 0 和 Astarin 802541 / Krondel 802543 对话（select1 的 SETPRO1），行 1 和“潘盖亚情报员”
# STR_DIC_N_GAb1_Ag_all 的 4 个中立变体（802544/802545/802546/802547）之一对话
# （select2 -> select2_1 的 SETPRO2），行 2 回接取 NPC Carley 802540 / Leaivink 802542
# 报告（select_success 的 SELECT_QUEST_REWARD）。旧定义把行 1/2 挂在接取 NPC 上直跳，
# 行 0 与情报员完全无路由。门禁 Batch41PangaiaFortressRowContractTest。
BATCH41_PANGAIA_FORTRESS_ROWS = {14220, 24220}

# 批次 42 登记（2026-09-22）：布鲁斯特豪宁献花族 4033（三行）。
# 行 0 采集 5 个 152000463 交给 Heintz 205155 换花环工作物品 182209042（select2 -> select2_1 ->
# select2_1_1 -> SETPRO1），行 1 在墓碑物件 700379 献花（select3 -> select3_1），行 2 回 Heintz
# 报告（select4 -> SELECT_QUEST_REWARD）。旧定义 SETPRO1 是 started 自环、墓碑 SELECT3_1 从
# started 直跳 reward，行 1/2 无状态。门禁 Batch42TombstoneFlowerRowContractTest。
BATCH42_TOMBSTONE_FLOWER_ROWS = {4033}

# 批次 43 登记（2026-09-22）：食人花解毒剂族 2239（三行）。
# 行 0 和 Vovetirn 203630 对话（select2 -> select2_1 -> select2_1_1 -> SETPRO1），
# 行 1 交 3 个 182203228 食人花外皮做解毒剂（select3 的 CHECK 链 + select3_2 的 SETPRO2，
# 发出工作物品 182203227），行 2 把解毒剂交给 Gilungk 203613（select4 的 SETPRO3 + 窗口 1）。
# 旧定义两个 NPC 都能接取/领奖，SETPRO1 直跳 reward。门禁 Batch43MalodorAntidoteRowContractTest。
BATCH43_MALODOR_ANTIDOTE_ROWS = {2239}

# 批次 44 登记（2026-09-22）：发光体玻璃瓶五行族 11118（Making Setzkiki Laugh）。
# 行 0 向 Cinisca 798963 征询（select1/SETPRO1）、行 1 交 20 个 182206794 换玻璃瓶 182206795
# （select2/CHECK）、行 2 在妹妹 Shulack_LF4_2 身边打开瓶子（select4）、行 3 和妹妹对话
# （SET_SUCCEED）、行 4 回哥哥 Shulack_LF4_1 798985 报告领奖（select_success）。
# 迁移前 Java handler 的 npc_ids 是 {798985, 798963, 798986}，正文却误写 798984（Shugo_LF4_5
# 且未注册监听）；本批修正 owner 为 798986，reward 投影 0 -> 4，并补 REWARD/var0=0 与
