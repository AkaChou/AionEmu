...
import os
AUDIT = Path(os.environ.get("AUDIT_CSV", ROOT / ".agents/summary/quest-load-fail/quest-order-audit-current.csv"))
...
OUTPUT = ROOT / ".agents/summary/quest-load-fail/unresolved-inventory.csv"

PAGE_NAME_RE = re.compile(r"^(?P<source>[^#]+)#(?P<page>\S+?) page-order")
RETAIL_TEMPLATES = {r["quest_id"]: r for r in csv.DictReader(
    (ROOT / ".agents/summary/quest-load-fail/retail-simple-templates.csv")
    .open(encoding="utf-8-sig", newline=""))}
EVIDENCE_NPC = {r["quest_id"]: r for r in csv.DictReader(
    (ROOT / ".agents/summary/quest-load-fail/data-driven-npc-evidence.csv")
    .open(encoding="utf-8-sig", newline=""))}
