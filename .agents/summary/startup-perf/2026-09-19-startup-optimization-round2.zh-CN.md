...
### 6.6 回归报告与结论（2026-09-19 23:13–23:15）

- 用户报告 IDE 启动变慢：`staticDataLifecycle` 9494ms（23:13:16）与 **11044ms**（23:13:48，解析 10474ms）。
- 排查：同期存在**两个 JVM 重叠**（pid 31939 于 23:12:56 启动、pid 32062 于 23:13:25 启动），
  加上同机进行的编译/JFR 测量，CPU 争抢污染了读数；此前 21:43–22:29 区间已见 ±400ms 的同配置波动。
- 独立测量（同一份分片代码，`-cp target/classes`）：23:02 = 解析 7281ms / 阶段 7514ms；
  "242 任务 → 10 批次"实验 23:15 = 解析 8575ms / 阶段 8795ms。
  → 差异由机器争抢主导，批次化**没有可证明的优势**。
- 处理：批次化实验回退，工作区回到已提交版本 `9166213fd`（分片 + patterns 并行）。
