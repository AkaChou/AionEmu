...
- 明细：`lombok/lombok_ctors_report.json`（构造器）与 `lombok/lombok_refactor_report.json`（访问器增量）。
...
- 逐方法明细：`lombok/lombok_refactor_report.json`；转换器：`lombok/lombok_refactor.py`（幂等，重跑 0 转换）。
- 改造前快照曾存放于 `.agents/lombok_backup/`、`.agents/lombok_backup2/`，编译验证通过后已按用户要求删除（约 43MB）。
