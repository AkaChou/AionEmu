...
<!-- pattern-metadata
status: CONFIRMED
scope: typed 任务定义 reward 节点投影（打包成 SM_QUEST_ACTION step 的 var0）vs legacy handler 进入 REWARD 时真正落盘的 step；重点是行号口径审计（QE-051）把 `reward = 末行索引 - 1` 的族误判成 MISSING_LAST_ROW 的场景；批次 28 起还覆盖同族的行阶梯（每一行一个 START/REWARD 状态，推进只由 legacy 事件触发）与旧存档自愈边的旧值选取
first_seen: 2026-09-22
last_verified: 2026-09-22
...
keywords: 领奖行、MISSING_LAST_ROW、legacy step、changeQuestStep、useQuestItem、SM_QUEST_ACTION、reward 投影、15300、25300、10100、20100、18805、28805、16800、26800、领奖阶阶梯、Archives、LEGACY_REWARD_STEP_IS_AUTHORITATIVE
-->
