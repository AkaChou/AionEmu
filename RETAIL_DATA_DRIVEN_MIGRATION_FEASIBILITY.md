...
> ScriptDLL NPC 对话传送矩阵已由 `generate_retail_script_transports.py` 从全部 `0x1b` 注册 shard、回调、`npcs.xml`、`WorldId.xml` 和 `Worlds/*/world.xml` 独立生成：818 个注册、757 个唯一回调、520 个传送调用；393 个注册不是传送，145 个完成整条路线语义闭包，280 个明确拒绝。端点与触发语义已分离统计，509 条静态端点可证明；全部调用至少按真端 API 归类为 `TELEPORT`，唯一有独立类型证据的 `IDNovice_Elevator_Lever_Up` 提升为 `LIFT`。生成器同时机械提取全部回调谓词、读取/调用操作并归并为 262 个结构族；矩阵仍不猜测未审计谓词、非传送调用和更细传送子类型，也不证明运行时消费者选择。

> AionEmu 门户矩阵只投影起点或终点属于 139 个生产副本世界的路线，开放世界内部传送不会进入副本迁移候选。当前共有 137 条候选：60 条已由 `PortalService` 精确承接，已无可直接转换条目；其余 14 条缺少运行时起点、38 条路线语义未闭合、7 条仍由不可重叠的特殊运行时消费者持有、18 条复杂 callback shape 继续明确拒绝。起点对账为 86 条匹配、37 条不匹配、14 条缺失，全部单独记录且不冒充传送语义门禁。本批次按 callback shape 批量闭合阵营、等级及 dialog 分支条件，修复十六进制 float 末位被误当后缀导致的端点坐标损坏，并让已有 `PortalReq.max_level` 与通用 `general` 对话传送 NPC 真正进入 PortalService 消费链；13 条同形定义完成转换，特殊门、任务物件和复活状态回调均未被覆盖。
