...
3. **stage+recovery 主战场（61+56）**：按副本组分批 ScriptDLL Pattern 恢复（RETAIL_PATTERN 45 先例证明管线可行）；recovery 依赖 stage 状态模型，随族推进。
   ...
   **独立静态矩阵状态更新（2026-07-27）**：`generate_retail_instance_stage_recovery_matrix.py` 不再从 `dimension_owners` 推导语义，只用 coverage 枚举 139 个生产世界并保留当前审计 owner；实际证据来自 condition-spawns 的真端源定位、可达 Pattern/旧 AI 操作、Handler 继承链、`InstanceRuntimeState` 和 `InstanceDeadlineScheduler`。矩阵覆盖 92 个 condition world、1,183 个条件变量、2,132 个可达 Pattern 阶段绑定、4,564 个 Pattern 计时绑定和 37 个旧 AI 阶段绑定；203 个变量生产者与 2 个传递 Pattern spawn 目标继续显式拒绝。当前 61 个 stage=HANDLER、56 个 recovery=HANDLER 中没有可直接删除 Handler 的批次：58 个世界保留 Handler，12 个世界存在硬证据缺口，其余 69 个已由外部 owner 持有。五个深渊宝箱房已机械归并为同一批，但仍缺 ScriptDLL 计时语义，不因源码同构而转换；coverage owner 暂不修改。
...
> ScriptDLL NPC 对话传送矩阵已由 `generate_retail_script_transports.py` 从全部 `0x1b` 注册 shard、回调、`npcs.xml`、`WorldId.xml` 和 `Worlds/*/world.xml` 独立生成：818 个注册、757 个唯一回调、520 个传送调用；393 个注册不是传送，145 个完成整条路线语义闭包，280 个明确拒绝。端点与触发语义已分离统计，509 条静态端点可证明；按真端 API 与回调结构证据归类为 498 条 `TELEPORT`、19 条 `ARENA_REENTRY`、2 条 `ITEM_GATED_TELEPORT` 和 1 条 `LIFT`。生成器同时机械提取全部回调谓词、读取/调用操作并归并为 262 个结构族；矩阵仍不猜测未审计谓词、非传送调用和运行时消费者选择。

> AionEmu 门户矩阵只投影起点或终点属于 139 个生产副本世界的路线，开放世界内部传送不会进入副本迁移候选。当前共有 137 条候选：84 条已由 `PortalService` 精确承接，已无可直接转换或未建模 callback shape；其余 14 条缺少运行时起点、38 条路线语义未闭合、1 条 Secret Safe Door 继续由钥匙对话消费者持有。起点对账为 86 条匹配、37 条不匹配、14 条缺失，全部单独记录且不冒充传送语义门禁。Aturam Lobby 的 4 条同形 alias 路线和 Bastion of Souls 的 2 条 dialog 104 阵营出口已批量切换到 `PortalDialogAI2 + PortalService`，旧端点和额外私服提示不再持有传送行为；2 条 Dredgion 钥匙传送也已按真端物品 ID、多点 alias 和专用失败消息 `1403685` 接管。竞技场重入的 16 个可达 NPC 已从同一 callback shape 机械派生票据检查 `0x300`、消耗 `0x1d0`、`JOIN_PLAY` 动作 `0x590`、失败对话 `0x188 → 1097` 和阶段 alias；运行时以通用门户扣费后调用 Crucible 的持久参与状态消费者，再按真端 alias 传送，不保留两个逐副本 Arbiter AI。

> 统一图状态更新 2026-07-27：当前已建模引用族共 79,251 条，resolved=71,063、rejected=543、fallback=7,645、unresolved=0、ambiguous=0。其中 ScriptDLL transport 新增 3,100 条端点、NPC、world 和 location_alias 引用，覆盖 520 次调用与 679 条路线；509 条端点闭合，170 条端点和 240 次无静态路线调用保持显式拒绝。AionEmu 门户矩阵已改读该统一图投影，不再直接以独立传送矩阵充当引用图。后续新增变量和技能槽等引用族时继续扩图，不把“当前图闭合”外推为尚未建模的语义已经存在。
