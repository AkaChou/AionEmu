...
> AionEmu 门户矩阵只投影起点或终点属于 139 个生产副本世界的路线，开放世界内部传送不会进入副本迁移候选。当前共有 137 条候选：45 条已由 `PortalService` 精确承接，2 条同形路线可转换；其余 14 条缺少运行时起点、38 条路线语义未闭合、5 条仍由不可重叠的运行时消费者持有、33 条 callback shape 尚未建模。起点对账为 86 条匹配、37 条不匹配、14 条缺失，全部单独记录且不冒充传送语义门禁。
