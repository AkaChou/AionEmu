...
> AionEmu 门户矩阵只投影起点或终点属于 139 个生产副本世界的路线，开放世界内部传送不会进入副本迁移候选。当前共有 137 条候选：68 条已由 `PortalService` 精确承接，已无可直接转换条目；其余 14 条缺少运行时起点、38 条路线语义未闭合、1 条 Secret Safe Door 继续由钥匙对话消费者持有、16 条竞技场重入 callback shape 继续明确拒绝。起点对账为 86 条匹配、37 条不匹配、14 条缺失，全部单独记录且不冒充传送语义门禁。Aturam Lobby 的 4 条同形 alias 路线和 Bastion of Souls 的 2 条 dialog 104 阵营出口已批量切换到 `PortalDialogAI2 + PortalService`，旧端点和额外私服提示不再持有传送行为；2 条 Dredgion 钥匙传送也已按真端物品 ID、多点 alias 和专用失败消息 `1403685` 接管。竞技场重入的票据消耗与 `JOIN_PLAY` 状态动作未闭合前仍不接管。
