...
2. `215457` 使用真端 AI `Elim_Octaside_Door`，被消灭后把 `IDElim_1F_StartRush` 设置为 `2`；实例脚本保留同一幂等兜底，覆盖旧刷怪或 AI 未接管的生命周期。
3. 条件组切换后，入口代理实体消失，服务端同时解除 entity 51 的 GEO 碰撞，客户端场景门进入可通行状态。实机验证结果是消灭 `215457` 后可以通过入口。
