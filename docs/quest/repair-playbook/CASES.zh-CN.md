...
## 8.46 接取分支的 `giveQuestItem` 在 typed 迁移中丢失导致任务道具根本不发放

- Pattern ID：`ACCEPT_TIME_WORK_ITEM_GRANT_LOST_ON_MIGRATION`。
- 代表任务：15545「[Learn Minion] Precious Ally Minion / 高级守护者的新同伴」（天族 66+ SIGNIFICANT，发起与领奖 NPC 835514）；同因任务 25545（魔族镜像 835515）、2266、3085、28808 按同一合同修复，不重复建立案例。
- 搜索症状：使用「下级宠物精灵契约书」召唤宠物精灵后任务没有往下、任务停在接取步、接取后背包里没有任务道具、交出页没有可交物品、`MinionService` 之后的任务推进分支从不触发。
- 玩家可见症状：玩家在 835514 接取 15545 后使用背包中的「下级宠物精灵契约书」成功召唤守护灵（动画与守护灵列表正常），但任务仍是 `START`，835514 也没有领奖对话；改用 GM 补发 `190080010` 后同一操作立即推进到 `REWARD`。
- 根因：`7e9f0316c`（1500 个 Java handler → typed XML）只迁移了状态与页面，没有迁移旧 handler 的**接取副作用** `giveQuestItem(env, 190080010, 1)`（原 `_15545A_New_Friend_For_A_Archdaeva.java:42` 的 `ACCEPT_QUEST_SIMPLE` 分支）。typed 引擎的 `<work-items>` 只负责「声明 + 完成时回收」（`QuestMutationPlanner.appendCompletionWorkItemCleanup`），接取发放必须显式写成 `NPC_START` 的 `<accept-actions><give-item/></accept-actions>`。更隐蔽的是物品名撞车：190080005/190080009/190080019/190089999 与任务专属的 190080010/190080011 在客户端都叫「下级宠物精灵契约书」，而 `MinionService#checkQuest` 只认「任务专属 id + status=START」才把任务写进 `REWARD`，因此玩家用任意其它同名契约书召唤成功都无法推进，看起来像“召唤了但任务不动”。
- 修复层：任务 XML（+ 全库只读审计与基线）。给旧 handler 的 `addOnQuestStart` owner 的 `NPC_START` 补 `<accept-actions><give-item item-id="<任务工作物品>" count="1"/></accept-actions>`，展开为 `QUEST_ACCEPT_1(1002)` 与 `QUEST_ACCEPT_SIMPLE(20000)` 两条 `unaccepted -> started` 路由（条件 `start-eligible`、after-commit `VISIBILITY_REFRESH` + 页面/关窗）；不改 `<work-items>` 声明、不改接取 owner、不引入每日志自愈发放（没有“未持有才发”的原子条件时会重复发放）。
- 修改文件：`src/main/resources/aion/data/static_data/quest_definition/quests/{15545,25545,2266,3085,28808}.xml`；`src/test/java/com/aionemu/gameserver/questEngine/definition/QuestMinionTutorialRetailAlignmentTest.java`；审计脚本/基线 `/.agents/summary/quest-15545-minion-accept-grant/`；验收记录 `.agents/summary/quest-acceptance/15545-2026-09-21-client-accepted.md`。
- 第一检查点：先确认玩家接取后有没有任务工作物品（`metadata.questWorkItems()` 声明的物品是否真的进过背包），再对比 `7e9f0316c^` handler 的接取分支与编译后两条接取路由上的 `GiveItem`；若物品在目录里只有任务定义与 `quest_data.xml` 出现（没有掉落/其它任务产出），即可判定为发放侧缺失，而不是玩家用错物品。
- 代表测试：`QuestMinionTutorialRetailAlignmentTest#archDaevaMinionChainGrantsItsOwnContractOnAccept`、`QuestMinionTutorialRetailAlignmentTest#legacyAcceptItemGrantsSurviveTheTypedMigration`。
