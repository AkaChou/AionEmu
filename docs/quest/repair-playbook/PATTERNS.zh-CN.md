...
| `NPC_DIALOG_ROUTE_GATE_COLLISION` | 转职或奖励状态仍要求选择、NPC 只显示“结束对话”、同一 NPC 多任务时正确任务路由被拦截 | NPC 对话索引可用宽 `TalkToNpc(npcId)` 键收集候选，但授权门控必须再按实际 action/dialog 匹配；同一动作存在已激活 MISSION/已接取 owner 时不能提前回退通用对话 | 对比宽 `hasRoutes` 与 `QuestEvent.matches` 的结果，列出同 NPC 的 owner、metadata category、当前状态和实际 route source/target | `59bba1a`、`QuestProductionDispatcherTest#ownerRouteLookupDoesNotConfuseAnotherTypedOwner` |
| `REWARD_PREVIEW_OPTIONAL_WORK_ITEM` | 已进入 `REWARD`，最终交任务显示 `HtmlPageId 1009`/load fail 或奖励页无响应，且旧任务物品可能已经不存在 | reward preview 的事务动作不能把可选/已被旧流程清理的工作物品按严格正数扣除；同时显式注册通用 `QUEST_SELECT(31)` 与直接 `USE_OBJECT(-1)` 到客户端真实 `SELECT5` 页，预览再用 `SELECT_QUEST_REWARD(1009)` 打开 page 5 | preview transition 的物品快照可缺失性、`RemoveItem` 可行性、action/page ID 空间、旧 handler 的“有则移除后仍进入 page 5”合同 | `aea256a29`、`Quest18600ClientDialogAlignmentTest#intermediateNpcPagesAndFinalRewardPagesFollowTheLegacyChain` |
| `REWARD_SELECTABLE_ITEM_DURABLE_LOWERING` | 任务已进入 `REWARD`，选择奖励后不完成、奖励不到背包或重复显示奖励页 | metadata 的 `SELECTABLE_ITEM` 只能作为选择描述；完成 owner 必须通过 `npc-complete`/planner 生成可持久化的 `ITEM`，不能把选择类型直接交给 durable reward port | 生产 XML 是否存在直接 `<grant-reward kind="SELECTABLE_ITEM">`、每个 choice/fallback 的 reward index、`GrantSelectedReward` 的 planner 降级结果和完成后事务动作 | `68d5786`、`Quest18602ClientDialogAlignmentTest#completesAtRaniniaWithConcreteSelectableRewards`、`QuestDefinitionCatalogManifestTest#repairedSelectableRewardQuestsDeliverEveryChoiceAsAConcreteItem` |
...
| `LEGACY_REWARD_STEP_PROJECTION_MISMATCH` | 进入 `REWARD` 后任务书空白、任务物品在背包但下一 NPC 不显示 | legacy handler 进入 `REWARD` 只更新 status，而 typed reward 节点或交接动作重写 packed var；旧存档还需要登录恢复到正确投影 | 旧 handler 的变量写入、reward 节点投影、`START -> REWARD` actions、`SM_QUEST_ACTION` packed vars 与客户端任务 step | `f6aff952a`、`LegacyRewardStepProjectionRegressionTest#keepsLegacyRewardEntryAtTheExistingPackedStep` |
...
| `REWARD_TITLE_AS_START_PREREQUISITE` | 接取任务 load fail、接受按钮无响应、正常角色被 `TITLE_MISSING` 拒绝 | metadata `title-id` 与奖励 `TITLE` 相同但 legacy `<quest>` 没有 quest-level `titleId`；`unaccepted -> started` 的 `start-eligible` 在接受动作前拒绝，未产生正确的 `sync -> page` 响应 | 对照 legacy quest-level `titleId` 与 `<rewards title>`，再查看 compiled `metadata.titleId` 和 `TITLE_MISSING` 分支；确认同一 ID 是奖励还是接取前置 | `880af62b7`、`Quest3926ClientDialogAlignmentTest#keepsTheRetailAcceptanceChainAndTreatsTitleAsAReward` |
...
| `CONFIRM_PAGE_HANDOVER_TIMING` | 选择后不进奖励、确认页按钮无响应、审计 `BUTTON_WITHOUT_ROUTE` 指向 `SELECT_REWARD(1009)` | 5.8 客户端在选择按钮后插入确认页：`SETPRO` 改纯翻页，物品检查/删除与进奖励移到确认页 `SELECT_QUEST_REWARD` 的互斥 `has-item` 分支（唯一优先级），缺物兜底回回落页 | 确认页 page id 与按钮动作、旧 handler 的检查/删除副作用、优先级唯一性 | `7268098e8`、`JavaHandlerFamilyDefinitionTest#robeDeliveryConfirmsTheChoiceThenSelectsAmongThreeItemGatedRewardBranches` |
| `STALE_LEGACY_BUTTON_SYMBOL` | 确认页按钮无响应、审计 `BUTTON_WITHOUT_ROUTE` 指向旧 handler 动作号 | 5.8 客户端改用了新按钮符号（如 `SETPRO8(10007)` 替代旧 `STEP_TO_11(10010)`），副作用保持不变的完成路由必须改挂客户端符号 | 客户端 HTML 页面按钮的动作 id、旧 handler 的期待动作与状态副作用 | `d966208a3`、`GelkmarosSelect8SymbolContractTest#select8_1ConfirmCompletesTheMissionThroughTheClientSymbol` |
| `DISABLED_INTERACTION_STAYS_UNROUTED` | 审计 `BUTTON_WITHOUT_ROUTE` 指向被注释禁用的旧交互页、克隆批次给非接取 NPC 配了接取页 | 旧 handler 明确注释禁用的交互保持无路由（页面作为集中管理例外记录），克隆到全部 NPC 的接取/奖励块按 `addOnQuestStart`/`addOnTalkEvent` 与领奖 owner 证据归位 | 旧 handler 的 register 列表与注释块、契约 start/end NPC、审计例外台账行 | `7268098e8`、`Quest30313RetailAlignmentTest#disabledPrismInteractionStaysUnroutedAndRewardClaimBelongsTo799225` |
| `CHECK_CONFIRMATION_PAGE_CONTRACT` | 上交后直接进奖励窗、客户端确认页不可达、审计 `CLIENT_PAGE_UNREACHED` 指向 check_user_item_ok/fail | 客户端在上交检查后插入确认页：成功分支显示 check_user_item_ok，确认按钮按 HTML 接管（FINISH 关闭走 REWARD 态 preview；1009 由 preview 独占）；不足显示 check_user_item_fail；确认页显示必须在状态 sync 之后 | 客户端 ok/fail 页按钮、39/1009 检查路由的 conditions 与 actions、sync 与页面包顺序 | `207e88649`、`AcceptAndConfirmationEntryContractTest#checkHandOverShowsTheClientConfirmationPageThenClosesOrClaims` |
| `AUTO_START_KEEPS_NONE_DIALOG_FREE` | 自动接取任务加对话接取入口后破坏既有流程、repeatable 别名迁移计数膨胀 | 区域/事件/道具自动接取任务的 NONE 态不得新增对话路由：接取由权威自动入口完成，select_none 简报页是集中管理例外；带 start-eligible 的新路由会被 repeatable 别名机制展开 | 该任务是否已有 enter-zone/enter-world/use-item 接取路由、NONE 态路由计数、别名展开后的迁移数 | `0095c6abf`、`AcceptAndConfirmationEntryContractTest#zoneAutoStartQuestsKeepUnacceptedFreeOfDialogRoutes` |
...
| `880af62b7` | 3926 魔道星老师的考验接受时弹 `Quest_Q3926.html` load fail | legacy 的 `rewards title="38"` 不是接取前置；移除 compact metadata 的 `title-id="38"`，保留奖励称号，并用客户端 `1002 -> 1003` 接取链测试锁定边界 |
...
| `772809b10` | 1005 实机抓包确立多步任务黄金定律与客户端废页归档 | 第 N 步对话下发 SELECT(N+1)，步进动作 SETPRO(N+1)，推步后以 0 关窗；台账将已闭环任务的客户端未实装废页提纯归档 |
| `106a69bda` | 道具起手任务 use-item 补全与第4页接取确认分类治理 | 补全 1197/1198/80008/80009 道具起手路由，清除 12 处历史遗留 QUEST_REFUSE_4 错误覆盖，台账清零第 4 页接取阻断（-68 行） |
| `bb0435fda` | 接取与拒绝页真实路由覆盖审定与多 NPC 起手分类提纯 | 审定 62 个任务 104 行接取/拒绝页阻断，证明 NPC_START 宏展开与显式 close-dialog 闭环事实，阻断清零（-104 行） |
| `待提交` | 纯动画任务与系统全局提示页提纯归档 | 归档 1000/2000 序幕纯过场动画任务对白残页、系统全局提示页（no_right/quest_complete/quest_failed_1）与深层分支页（-121 行） |
