...
> 状态：阶段 0/1、共有 Quest 的 `cannot_share` / `can_report` 同步、`can_report` 运行能力、首批 18 个 Quest 静态发布、第二批 6 个要塞自动领奖 Quest、第三批 10 个普通生产 Quest、第四批 `2585`、第五批 `21224`、C batch 01 `1871/2871`、C batch 02 `15098/25099`、C batch 03 `14210/24210`、C batch 04 `1867/1868/1869/2868`、D batch 01 `18604/28604`、D batch 02 `30810`、D batch 03 `18744/28744`、D batch 04 `11295` 运行闭包、D batch 05 `10070-10073/20070-20073` 与 D batch 06 `14080/14081/14090/14091/24080/24081/24090/24091` 隔离终审、护送/保护/动态生成 AI 审计，以及 `18412/28412`、`41600-41614`、`80315/80321` 隔离终审均已完成（2026-07-19）。当前已发布 52 个 Quest，35 个 Quest 已有明确隔离结论，AI 生产阻断为 0。本文方案仅涉及服务端 Quest 模板和服务端行为同步，不修改、生成或重新打包客户端 Quest.pak；真实入口与实际运行验收状态单独列出。
...
- 39 个疑似测试/开发任务已按“不猜值、可疑即隔离”规则叠加 E 隔离。第二批 6 个、第三批 10 个、第四批 `2585`、第五批 `21224`、四个 C 批次与 D batch 01/02/03/04 的模板和行为已闭包，不再属于堵点；D batch 03 发布 `18744/28744`，并将无法从客户端模板与真端行为形成一致配对的 `18412/28412` 固定隔离；D batch 04 发布 `11295`，保留固定客户端 `check_item1_1=quest_18411a 2` 的物品门禁，不向普通天族角色放宽。D batch 05 的 `10070-10073/20070-20073` 被等级 999 前置和缺失的 `LDF4a/LDF4b` 可加载关卡阻断；D batch 06 的八个 58-60 级任务同样依赖最低等级 999 的 `14071/24071` 和不可加载的 `LDF4b`。其中 `14081/24081` 虽恢复出真端区域、NPC、进度、领奖和完成回调，仍不能跨过固定客户端没有 `Levels/LDF4b` 的硬阻断；其余六个也没有独立运行闭包，现均正式隔离。旧“剩余 17”口径重复包含已在 D batch 03 隔离的 `18412/28412`，去重后本批前实际未决 15 个，本批处理 8 个后剩余 7 个：`2010/9510/9615/9652/12999/15097/20015`。当前真正耗时点转为这 7 个 D 类缺失行为、已发布 Quest 的真实运行验收和 `50019/51019/80341` 的入口取证。
...
- AI 与 Quest 按事件链结合，不把 NPC AI 模板直接合并进 Quest。当前 18 个使用 `defaultStartFollowEvent` 的护送 handler 为 18/18 `closed`；保护类保留 1 条 `no_evidence` 信息状态；真端 57 条动态生成动作中生产范围 46 条全部 `closed`、等级 999 范围 11 条为 `isolated`；第二批要塞自动领奖击杀链为 `siege_report_hunt closed=6`，第三批 10/10、第四批 1/1、第五批 1/1、前三个 C 批次各 2/2、C batch 04 为 6/6、D batch 01 为 2/2、D batch 02 为 1/1、D batch 03 为 2/2、D batch 04 为 1/1 Quest 运行闭包 `closed`，生产阻断为 0。D batch 01 不涉及护送、保护或动态生成，但尸体恢复 AI 已审计为“必须有 `18604/28604` 任务记录且物品缺失”，旧的无任务发物品路径已删除；D batch 02/03/04 只含角色状态、进世界和电影结束事件，AI 生命周期为 `not_applicable`。D batch 05/06 各 8 条世界绑定任务均为 `isolated`；D batch 06 的 `14081/24081` 已额外冻结真端多阶段回调，但在恢复世界前仍须重做物品使用区、感知区、动态 NPC/对象生命周期、清理、失败恢复和 Quest 推进闭包。
...
本轮已经完成权威源、候选范围、测试隔离、名称解析、客户端目标覆盖、转换器实现、首批静态发布、第二批要塞自动领奖闭包、第三批普通生产 Quest、第四批 `2585` 前置兼容、第五批 `21224` 末次扩展奖励、四个 C 批次、D batch 01/02/03/04 闭包和 D batch 05/06 隔离终审。首批“静态发布完成”仍只表示模板/行为文件已加载并通过结构校验；后续已发布批次已额外闭包自然接取入口、目标、状态推进和运行所需资源，仍需在真实在线角色上做最终运行验收。
...
| D batch 06 `14080/14081/14090/14091/24080/24081/24090/24091` | 已隔离 | 两条链由最低等级 999 且服务端不存在的 `14071/24071` 起始，全部依赖 `600030000/LDF4b`。`14080/24080` 只有固定客户端掉落目标，`14081/24081` 虽有真端名称绑定、注册、进度、领奖和完成回调，固定客户端仍无 `Levels/LDF4b`、`Level.pak`、`PathFind.pak`；AionEmu 无 world/GEO/spawn map，13 个目标只有模板没有 spawn。`14090/24090/14091/24091` 仅恢复到入口别名状态检查，不能形成独立推进闭包 | 否，不发布、不修改客户端；去重后生产 D 类剩余 7 个 |
...
| D 类行为缺失 | 未关闭 | D batch 01/02/03/04 已恢复 6 个；D batch 03 已隔离 `18412/28412`，D batch 05/06 又各隔离 8 个世界绑定任务。旧“剩余 17”重复计入 `18412/28412`，去重后本批前实际 15 个、本批后剩余 7 个：`2010/9510/9615/9652/12999/15097/20015` | 否，属于后续阶段 |
...
因此 18 个首批 Quest 已具备静态发布条件并已发布，但只有接取 NPC 已存在且流程实测通过的 Quest 才能升级为“端到端迁移完成”。上述 11 个入口受阻 Quest 必须保持明确状态，不能用 GM 命令强制接取后的成功推进替代自然接取验收；其中 `41615-41622` 已按地图级隔离登记，不再等待服务端单方面“修复”。第二批 6 个、第三批 10 个、第四批 `2585`、第五批 `21224`、四个 C 批次和 D batch 01/02/03/04 不再命中这些入口堵点，其静态和运行资源闭包已完成；D batch 05/06 不进入发布集。

LDF4b 终审已由 `/Users/mc/PycharmProjects/aion_drop/scripts/quest_migration_audit.py` 固化为 `target/quest-migration/reports/ldf4b_isolation.json`，当前 v20 仍保持该隔离基线。结论证据如下：
...
- 一级候选的 42 个 D 类中有 11 个 `DataDriven Empty` 任务已隔离；D batch 01/02/03/04 已恢复 `18604/28604/30810/18744/28744/11295`，`18412/28412` 因证据无法形成一致配对保持隔离。D batch 05 隔离 `10070-10073/20070-20073`，D batch 06 隔离 `14080/14081/14090/14091/24080/24081/24090/24091`。旧“剩余 17”重复计入 `18412/28412`；去重后当前只剩 `2010/9510/9615/9652/12999/15097/20015` 7 个 D 类需要从其他真端脚本、反编译证据或可验证流程恢复。
...
因此候选范围、39 个疑似测试/开发任务的 E 隔离、名称解析、客户端目标核对、`cannot_share` 同步、`can_report` 通用能力、BM 兼容 no-op、第二至五批、C batch 01/02/03/04 与 D batch 01/02/03/04 Quest 运行闭包，以及 D batch 05/06、`18412/28412`、`41600-41614`、`80315/80321` 的隔离终审已经完成。当前最大耗时转为 7 个 D 类行为恢复、已发布 Quest 的真实运行验收，以及 `50019/51019/80341` 的入口取证；LDF4a/LDF4b 相关任务维持已记录的隔离/静态发布限制，不消耗当前开发阶段时间。`2055` 和其余 3,406 个等级 999 Quest 全部后置。
...
- 旧“剩余 17”去除已在 D batch 03 隔离的 `18412/28412` 后，本批前实际未决 15 个；D batch 06 后只对 `2010/9510/9615/9652/12999/15097/20015` 7 个 D 类继续恢复证据。11 个 `DataDriven Empty` D 类维持 E 隔离。
...
状态：进行中。D batch 01/02/03/04 已完成发布闭包，D batch 05/06 已完成隔离终审，去重后生产 D 类剩余 7 个；验收要求仍是不存在被统计为已迁移但无法完成、或因旧实现残留走错流程的 Quest。
...
- 总 Quest 数、共有/独有/扩展数；
- A/B/C/D/E 数量；
- 一级候选 133、原 E 隔离 39、生产审计池 94、首批 18、第二批 6、第三批 10、第四批 1、第五批 1、C batch 01/02/03 各 2、C batch 04 新模板 4、D batch 01 新模板 2、D batch 02 新模板 1、D batch 03 新模板 2、D batch 04 新模板 1、当前剩余一级候选 81，以及 LDF4b 地图级隔离 15、活动物品任务隔离 2、D batch 03 配对隔离 2、D batch 05/06 世界链各隔离 8、正式隔离合计 35 的基线；
...
| D batch 06 后正式隔离 / 去重后生产 D 类剩余 / AI 阻断 | 35 / 7 / 0 |
| v20 受管输出 / 残留旧输出 | 55 / 0 |
| 活动物品任务明确隔离 | 2 |
...
- [x] 已发布批次的候选 Quest XML、候选行为 XML、`event.xml` 和正式行为 XML 通过 XSD；当前 Quest 总数为 6,476，第二批新增行为 6 条、第三批新增行为 2 条、第四和第五批复用现有行为，C batch 01 复用 `report_to_many`，C batch 02/03 复用 `report_to`，C batch 04 新增 4 个具体 Java handler、2 条 `monster_hunt` 和 8 个区域，D batch 01 新增 1 个共享基类与 2 个薄 handler，D batch 02 新增 1 个独立 Java handler，D batch 03 新增 1 个共享基类与 2 个薄 handler，D batch 04 新增 1 个独立 Java handler；已发布的四个 D 批次均不新增 XML 行为，D batch 05/06 不生成任何正式资源。
...
- [x] 迁移器 v20 清理自检通过；生成前删除被替代的旧 Quest XML、Quest 行为 XML 和报告，不删除历史取证目录；PAK 临时证据只进系统临时目录；受管允许清单 55 个文件全部存在、残留旧输出为 0，连续两次完整审计摘要一致。
...
17. 去除已隔离的 `18412/28412` 重复计数后，只对 `2010/9510/9615/9652/12999/15097/20015` 7 个生产 D 类恢复行为证据；11 个 `DataDriven Empty` D 类维持 E 隔离。
...
可以较高自动化完成的是 Quest 模板字段、常规奖励、限制、掉落、前置和严格 B 类行为；四个生产 C 批次和 D batch 01/02/03/04 已经闭包，D batch 05/06 已按固定客户端和真端证据正式隔离，当前不能自动承诺完整的是去重后剩余 7 个缺失行为证据的 D 类任务。最后 15 个生产 A/B Quest 已按固定客户端关卡资产缺失正式隔离，不再属于待开发承诺。动态宏、名称索引、faction、`cannot_share`、`can_report`、`bm_restrict_category`、`quest_permitted_worlds` 的本批行为门控和满足条件的末次 `reward_repeat_count` 已确认不再阻塞当前发布。
...
当前结论是：技术路线可行，转换器、分阶段覆盖基线、共有 Quest 分享/上报字段同步、自动领奖通用能力、BM 兼容 no-op、首批静态发布、第二批 6 个要塞自动领奖 Quest、第三批 10 个普通生产 Quest、第四批 `2585`、第五批 `21224`、四个 C 批次和 D batch 01/02/03/04 运行闭包、护送和动态生成 AI 审计，以及 D batch 03 配对隔离、D batch 05/06、LDF4b/活动物品任务隔离终审均已落地。动态生成 46 条生产动作、`siege_report_hunt` 6 条链、第三批 10 条、第四批 1 条、第五批 1 条、前三个 C 批次各 2 条、C batch 04 的 6 条、D batch 01 的 2 条、D batch 02 的 1 条、D batch 03 的 2 条和 D batch 04 的 1 条运行闭包均已清零阻断；D batch 05/06 均为 `8/8 isolated`，`18412/28412`、`2055`、`41600-41614`、`80315/80321` 与其余等级 999 内容按证据隔离。52 个 Quest 已从候选转为正式模板，发布后剩余一级候选为 81，正式隔离总数为 35。下一步处理去重后剩余的 `2010/9510/9615/9652/12999/15097/20015` 7 个 D 类缺失行为证据；并行完成已发布 Quest 的真实在线运行验收和 `50019/51019/80341` 的入口取证。保护类在具体 handler 和事件分发入口均为 0 的现状下不得猜测实现；3,406 个等级 999 Quest 全部后置。
