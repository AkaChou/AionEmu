...
- 权威客户端、基础客户端和 58Server 都包含同一组 10,035 个 Quest ID。迁移前服务端有 6,424 个，双方共有 6,410 个，客户端独有 3,625 个，另有 14 个服务器扩展 Quest；前五批、四个 C 类批次与四个 D 类批次共发布 52 个后服务端为 6,476 个，双方共有 6,462 个，客户端剩余独有 3,573 个，服务器扩展仍为 14 个。
...
- 39 个疑似测试/开发任务已按“不猜值、可疑即隔离”规则叠加 E 隔离。第二批 6 个、第三批 10 个、第四批 `2585`、第五批 `21224`、四个 C 批次与 D batch 01/02/03/04 的模板和行为已闭包，不再属于堵点；D batch 03 发布 `18744/28744`，并将无法从客户端模板与真端行为形成一致配对的 `18412/28412` 固定隔离；D batch 04 发布 `11295`，保留固定客户端 `check_item1_1=quest_18411a 2` 的物品门禁，不向普通天族角色放宽。D batch 05/06 的 16 个世界绑定任务已分别按等级 999 前置、固定客户端缺少可加载 `LDF4a/LDF4b` 关卡和缺少独立行为闭包正式隔离。D batch 07 对最后 `2010/9510/9615/9652/12999/15097/20015` 7 个生产 D 类完成终审：客户端基础/China 模板一致，客户端 3 个行为源和真端 9 个通用行为 XML 均零覆盖；`9615` 只恢复两个未知引擎调用片段，`12999` 只有 NPCServer 加载期 AddQuest 特例，电影资产没有权威 Quest 绑定，社区 `9510` 行为又与真端计时器用途冲突，因此 7/7 正式隔离。生产 D 类未决由 7 降为 0；当前真正耗时点只剩已发布 Quest 的真实运行验收和 `50019/51019/80341` 的入口取证。
...
| 固定客户端 `CutScene.pak` | `05118fc2f45f8cbebf0d356b560e4c76ba02b6b81c9ed6741ba6426db63d75df` |
...
| 真端 `ScriptDLL64.dll` | `99b87b9b291bcee4c01a9658784f9ca3e7751806dbffe9c9225ceb8f25f5d779` |
...
| D batch 01 发布后当前 `quest_data.xml` | `6d23d771a8990dfd0bad0a19f71ec2f2c384b7d303d4a836b3fb118ca15153c3` |
...
本轮已只读解包基础、China Quest 和 CutScene PAK 完成统计与电影映射核对，没有修改客户端文件。基础、China 和 CutScene 的临时解包统一使用系统临时目录，审计结束自动删除；D batch 03/04 不保留批次级 CutScene 临时证据目录。固定客户端 Nightmare Circus `Level.pak` 仍只解包到 `/Users/mc/PycharmProjects/aion_drop/target/quest-migration/client-level-audit/` 用于出生证据核对，没有回写或重新打包客户端。
...
迁移前候选基线与发布后运行数据基线必须分开使用，不能把已发布的 52 个 Quest 再次统计为候选。

| 项目 | 迁移前 | 首批发布后 | 第二批发布后 | 第三批发布后 | 第四批发布后 | 第五批发布后 | C batch 01 发布后 | C batch 02 发布后 | C batch 03 发布后 | C batch 04 发布后 | D batch 01 发布后 | D batch 02 发布后 | D batch 03 发布后 | D batch 04 发布后 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 固定客户端/真端 Quest | 10,035 | 10,035 | 10,035 | 10,035 | 10,035 | 10,035 | 10,035 | 10,035 | 10,035 | 10,035 | 10,035 | 10,035 | 10,035 | 10,035 |
| 当前 Quest | 6,424 | 6,442 | 6,448 | 6,458 | 6,459 | 6,460 | 6,462 | 6,464 | 6,466 | 6,470 | 6,472 | 6,473 | 6,475 | 6,476 |
| 双方共有 | 6,410 | 6,428 | 6,434 | 6,444 | 6,445 | 6,446 | 6,448 | 6,450 | 6,452 | 6,456 | 6,458 | 6,459 | 6,461 | 6,462 |
| 客户端独有 | 3,625 | 3,607 | 3,601 | 3,591 | 3,590 | 3,589 | 3,587 | 3,585 | 3,583 | 3,579 | 3,577 | 3,576 | 3,574 | 3,573 |
| 当前独有 | 14 | 14 | 14 | 14 | 14 | 14 | 14 | 14 | 14 | 14 | 14 | 14 | 14 | 14 |
| 最低等级不高于 75 | 219 | 201 | 195 | 185 | 184 | 183 | 181 | 179 | 177 | 173 | 171 | 170 | 168 | 167 |
| 一级候选 | 133 | 115 | 109 | 99 | 98 | 97 | 95 | 93 | 91 | 87 | 85 | 84 | 82 | 81 |
| 理论并集 | 10,049 | 10,049 | 10,049 | 10,049 | 10,049 | 10,049 | 10,049 | 10,049 | 10,049 | 10,049 | 10,049 | 10,049 | 10,049 | 10,049 |
...
本轮已经完成权威源、候选范围、测试隔离、名称解析、客户端目标覆盖、转换器实现、首批静态发布、第二批要塞自动领奖闭包、第三批普通生产 Quest、第四批 `2585` 前置兼容、第五批 `21224` 末次扩展奖励、四个 C 批次、D batch 01/02/03/04 闭包和 D batch 05/06/07 隔离终审。生产 C/D 类未决均为 0。首批“静态发布完成”仍只表示模板/行为文件已加载并通过结构校验；后续已发布批次已额外闭包自然接取入口、目标、状态推进和运行所需资源，仍需在真实在线角色上做最终运行验收。
...
| D batch 01 `18604/28604` | 已关闭 | 固定客户端模板优先；真端区域、7 个反编译函数、3 个 NPC spawn、世界/GEO、共享 Java handler、石像对话和尸体恢复门禁全部闭包，旧无任务发物品路径及重复行为均不存在 | 否，已发布 |
| D batch 02 `30810` | 已关闭 | 固定客户端模板优先；`fun_648/875.cpp` 证明 10 级隐藏接取、电影 `36`、电影结束完成和中断后进世界补完成；正式资源为 1 条模板、1 个 Java handler、0 条旧 XML 行为，旧模板和重复注册均不存在 | 否，已发布 |
| D batch 03 `18744/28744` | 已关闭 | 固定客户端模板和分享语义优先；真端 DLL 与 `fun_006/725.cpp` 证明进入 `300610000` 按种族播放电影 `912/913`、接取、电影结束完成及中断后重播；正式资源为每 ID 1 条模板、1 个薄 handler、0 条旧 XML 行为 | 否，已发布 |
| D batch 03 `18412/28412` | 已隔离 | 客户端模板与真端入口/电影证据不能形成一致发布对；模板、Java handler、XML 行为均保持 0，不猜测复用 `18744/28744` 流程 | 否，不发布 |
| D batch 04 `11295` | 已关闭 | 固定客户端模板与 `check_item1_1=quest_18411a 2` 决定门禁；真端 DLL 注册/回调字节证明进入 `320150000` 接取、电影结束完成；现有副本脚本已在巢穴区域播放电影 `488`，正式资源为 1 条模板、1 个 Java handler、0 条旧 XML 行为 | 否，已发布；不修改客户端和实例脚本 |
| D batch 05 `10070-10073/20070-20073` | 已隔离 | 两条 60 级任务链分别依赖最低等级 999 且服务端不存在的 `10064/20064`；`10072/20072` 目标在 `LDF4a`，`10073/20073` 四只目标在 `LDF4b`。固定客户端仅保留 `600020000/600030000` 元数据和周边资源，没有对应 `Levels`/寻路资产；AionEmu 无 world/GEO/spawn，真端 DLL 对 8 个 Quest ID 和 14 个任务物品均为 0 次命中 | 否，不发布、不修改客户端；当时记录 17，batch 06 已去重修正 |
| D batch 06 `14080/14081/14090/14091/24080/24081/24090/24091` | 已隔离 | 两条链由最低等级 999 且服务端不存在的 `14071/24071` 起始，全部依赖 `600030000/LDF4b`。`14080/24080` 只有固定客户端掉落目标，`14081/24081` 虽有真端名称绑定、注册、进度、领奖和完成回调，固定客户端仍无 `Levels/LDF4b`、`Level.pak`、`PathFind.pak`；AionEmu 无 world/GEO/spawn map，13 个目标只有模板没有 spawn。`14090/24090/14091/24091` 仅恢复到入口别名状态检查，不能形成独立推进闭包 | 否，不发布、不修改客户端；交由 batch 07 收尾剩余 7 个 |
| D batch 07 `2010/9510/9615/9652/12999/15097/20015` | 已隔离 | 基础/China 模板逐节点一致；客户端 `quest_monster.csv`、`quest_script_monster.csv`、`data_driven_quest.xml` 与真端 9 个通用行为 XML 全部零覆盖。`2010` 明确 UNUSED；`9510` 真端开发注释为计时器专用且禁止删除，社区 `report_to` 无真端行为支持；`9615` 仅有两个未知 `+0x3d0` 引擎调用；`9652` 无潘盖亚 PVP 入口/推进/结算；`12999/15097/20015` 只有用途注释、加载期特例或电影候选，没有权威 Quest 绑定和完成回调 | 否，7/7 不发布、不猜值、不修改客户端；生产 D 类剩余 0 |
...
| D 类行为缺失 | 已关闭生产未决 | D batch 01/02/03/04 已恢复 6 个；D batch 03 隔离 `18412/28412`，D batch 05/06 各隔离 8 个世界绑定任务，D batch 07 将最后 7 个无完整闭包任务正式隔离。生产 D 类未决为 0，后续只有新权威证据出现时才重开单个任务 | 否 |
...
因此 18 个首批 Quest 已具备静态发布条件并已发布，但只有接取 NPC 已存在且流程实测通过的 Quest 才能升级为“端到端迁移完成”。上述 11 个入口受阻 Quest 必须保持明确状态，不能用 GM 命令强制接取后的成功推进替代自然接取验收；其中 `41615-41622` 已按地图级隔离登记，不再等待服务端单方面“修复”。第二批 6 个、第三批 10 个、第四批 `2585`、第五批 `21224`、四个 C 批次和 D batch 01/02/03/04 不再命中这些入口堵点，其静态和运行资源闭包已完成；D batch 05/06/07 不进入发布集。

LDF4b 终审已由 `/Users/mc/PycharmProjects/aion_drop/scripts/quest_migration_audit.py` 固化为 `target/quest-migration/reports/ldf4b_isolation.json`，当前 v21 仍保持该隔离基线。结论证据如下：
...
D batch 01 发布 `18604/28604`。固定客户端决定模板、中文名称和 Quest ID：两者均为 37 级 `NON_COUNT`、不可分享、奖励经验 `150` 与物品 `164000141`；真端 `Map/Worlds/idcromede/world.xml` 提供 `300230000` 内 11 点精确多边形，`fun_617/697/702/722/887/892/906.cpp` 证明区域自动接取、石像 `STEP_TO_3` 完成和尸体恢复门禁。服务端复用现有 Quest 引擎，以一个共享基类和两个 ID 薄 handler 实现，不增加第二套 XML 行为；`206154/730309/700961` 模板与 spawn、世界/GEO、银刃罗坦物品模板全部闭包。`Grave_Robber_CorpseAI2` 已删除“没有任务记录也能发物品”的旧路径，恢复物品必须满足存在 `18604/28604` 记录且背包缺少该物品。完整审计为 `6472 / 6458 / 3577 / 85`、`released=true`、`common_diff_rows=0`、运行闭包 `2/2 closed`、未解析字段/引用 `0/0`、AI 阻断 `0`；护送/保护/动态生成对本批为 `not_applicable`。候选模板、D batch 01 报告和发布清单 SHA-256 分别为 `3c1db91402f54ae8281b86faf7f90b5ed71e629fdbd680b10cecd90dd2fde450`、`313e451ff44248bde512eeb1ac8a45f1ec9bebc1c3471ea62313bbf25bb93103`、`14840e6821dc18091e4242e0214b187edbaa3d4123c6752be98a58e3472ac2b0`。

D batch 02 发布 `30810`。固定客户端决定模板：`nameId=1803754`、最低等级 10、单次、不可分享、可放弃、`PC_ALL/QUEST`；真端 `fun_648.cpp` 与 `fun_875.cpp` 证明角色达到 10 级时隐藏接取、播放电影 `36`、电影结束后完成，以及电影中断重登时直接补完成。服务端使用一个最小 Java handler 注册等级提升、进世界和电影结束事件，不涉及 NPC、spawn、护送、保护或动态生成。发布前扫描证明旧 `30810` 模板和旧 XML/Java 行为均不存在；发布后正式资源固定为 1 条模板、1 个 handler、0 条 Quest XML 行为，重复注册门禁为 `single_template=true`、`single_handler=true`、`no_legacy_xml_behavior=true`。完整审计为 `6473 / 6459 / 3576 / 84`、`released=true`、`common_diff_rows=0`、运行闭包 `1/1 closed`、未解析字段/引用 `0/0`、AI 阻断 `0`；候选模板、D batch 02 报告和发布清单 SHA-256 分别为 `69b087680fc31ca372ce7844caeb06d53abac381dd08afd35d505d6700dd3ec6`、`e8839b01407784dc47247ae35bc5c0624a3ad4eadd531387cbe32b174d9c218d`、`17eb30432bd8087fa38e221827f76f17f577e4ca0338767d42125101e99ef9a2`。

D batch 03 发布 `18744/28744`，隔离 `18412/28412`。固定客户端决定模板与分享语义：`18744` 为“阿比索提供的勒门图的情报”、`nameId=1800942`、ELYOS；`28744` 为“普罗库拉提供的勒门图的情报”、`nameId=1800950`、ASMODIANS；两者均为 60 级、单次、可分享、不可放弃的 `QUEST`，客户端原始 `cannot_share` 为空，服务端显式保存为 `false`。固定客户端 `CutScene.pak` 唯一映射 `18744 -> 912 -> CS_ID_132 -> cs_id_132.seq -> 804716/Aviso`、`28744 -> 913 -> CS_ID_133 -> cs_id_133.seq -> 804717/Procura`；两段电影 `EndTime=56`，演员坐标均为 `818,932,1207.4312`。真端 `ScriptDLL64.dll` 的唯一入口签名、`fun_006.cpp` 的电影结束注册和 `fun_725.cpp` 的 `FUN_180c855f0/FUN_180c859e0/FUN_180c85a30` 共同证明：进入 `300610000` 时按种族接取并播放电影，未完成任务再次进入世界重播，电影结束后完成。服务端只新增一个共享基类和两个 ID 薄 handler，不修改客户端、不修改实例脚本、不增加 Quest XML 行为；护送、保护、动态生成均为 `not_applicable`。发布门禁固定为每个发布 ID 1 条正式模板、1 个 Java handler、0 条 Quest XML 行为，`18412/28412` 的模板/Java/XML 全为 0，旧受管生成物由 v17 allowlist 在生成前删除。完整审计为 `6475 / 6461 / 3574 / 82`、`released=true`、`common_diff_rows=0`、运行闭包 `2/2 closed`、未解析字段/引用 `0/0`、AI 阻断 `0`；候选模板、D batch 03 报告和发布清单 SHA-256 分别为 `2e882c32dcbeb789780c898b91599c80e74fc50f899187c3f8ae0ab386809b9f`、`d46b9cf10044cdbf33950808783a29c11df87b469bc5a7fd0049620c79e33de8`、`1afb5699029a1c9a58200d0dbc5b8e89e75518661f9c17546f1589ad37c9f3b6`。

D batch 04 发布 `11295`。固定客户端决定模板为“动画播放用隐藏任务”、`nameId=1129656`、最低等级 50、最多 255 次、不可分享、可放弃、ELYOS `QUEST`；客户端原始 `check_item1_1=quest_18411a 2` 已解析为 `182215009 × 2`，该条件无法放入当前模板，必须由 Java 进世界门禁显式保留。关联任务 `18411` 在固定客户端最低等级为 999，因此普通新角色不能自然取得这些物品；本批只兼容仍持有旧物品的角色，禁止把门禁放宽为所有天族进入副本都自动接取。真端 `ScriptDLL64.dll` 在 `0x1805d5c90` 注册 `event=0x12/world=320150000/FUN_180c852d0`，回调以 `questId=11295` 调用接取虚表槽 `0x148`；在 `0x1805d5e90` 注册 `event=0x13/world=320150000/FUN_180c85770`，回调以同一 Quest ID 调用完成虚表槽 `0x160`。由于两个短回调未出现在反编译正文中，v18 同时冻结 DLL SHA-256、两条注册入口和两条回调函数字节签名，并继续冻结相邻恢复源 `fun_006/725.cpp`。固定客户端 `CutScene.pak` 唯一映射 `488 -> CS_ID_088 -> cs_id_088.seq`，`EndTime=21`；当前 `PadmarashkaCaveInstance` 已在 `PADMARASHKAS_NEST_320150000` 调用 `sendMovie(player, 488)`，世界 `320150000`、GEO、spawn、zone 和物品模板均存在。因此服务端只新增 1 条正式模板和 1 个 handler：进入世界时要求 world/race/level/item 全部满足后调用 `QuestService.startQuest`，电影 `488` 结束时只允许 `START` 状态完成，不再次检查或扣除物品；不发送电影包，不修改客户端，不修改实例脚本，不增加 Quest XML 行为。护送、保护和动态生成为 `not_applicable`。完整审计为 `6476 / 6462 / 3573 / 81`、`released=true`、`common_diff_rows=0`、运行闭包 `1/1 closed`、未解析字段/引用 `0/0`、AI 阻断 `0`；候选模板、D batch 04 报告和发布清单 SHA-256 分别为 `4dd13ea91f230b7caa9d3134dbbfab16a81ae9a71cf8c455184e445116a0670e`、`68ed83f9440b526118a86822d43cf50b5075c416d3456e9d061ab336ce8ae700`、`e4f96720918510567edb12cb9bb630cee71cef67431987084fc3d7c2ea5a5b08`。

D batch 05 隔离 `10070-10073/20070-20073`。基础与 China 模板逐节点摘要一致，8 个 Quest 均为 60 级，并形成 `10064 -> 10070 -> 10071 -> 10072 -> 10073` 和 `20064 -> 20070 -> 20071 -> 20072 -> 20073` 两条链；链首 `10064/20064` 在固定客户端最低等级均为 999，当前服务端也不存在。固定客户端 `quest_monster.csv` 将 `10072/20072` 的收集目标固定为 `LDF4a_Monster_M_Aurora_58_An`，`quest_script_monster.csv` 将 `10073/20073` 的四个分段击杀目标固定为 `LDF4b_T1_Skeleton_Named_58_An`、`LDF4b_T2_Yun_Named_59_An`、`LDF4b_T3_Fanatic_Named_59_An`、`LDF4b_T4_Yun_Ghost_Named_60_An`。客户端 `world.pak` 仍保留 `LDF4a=600020000/93 subzones` 和 `LDF4b=600030000/116 subzones` 以及 loading、地图、skybox、dialogs、sound 周边资源，但 `Levels/` 中没有 `LDF4a/LDF4b` 目录、`Level.pak` 或 `PathFind.pak`；`idtiamat_1/2` 对应的 `300510000/300520000` 副本资源虽存在，不能替代任务要求的开放世界。AionEmu 同样只有两个副本的 world/GEO/spawn，缺少 `600020000/600030000` 的活动 world、GEO、spawn，五个任务怪仅有模板没有出生。真端 `ScriptDLL64.dll` 对 8 个 Quest ID 和 `182213241-182213254` 14 个任务物品的 little-endian 32 位常量命中均为 0；`300510000` 的 4 次命中属于其他流程，`300520000/600020000/600030000` 均为 0，不能形成该批行为闭包。旧社区 handler 仅作为检索索引，因包含 TODO、NPC 注册/目标不一致、未完成传送和对缺失世界的依赖而不采用。迁移器 v19 因此只生成 `d_batch_05_isolation.json`，不生成模板、Java handler 或 XML 行为；AI 状态为 `8/8 isolated`，未来若恢复动态 NPC/怪物生成，必须重新验证触发、生命周期、归属、成功清理、失败恢复和 Quest 推进。正式隔离总数由 19 增至 27，生产 D 类由 25 减至 17，Quest 覆盖仍为 `6476 / 6462 / 3573 / 81`，不修改客户端。

D batch 06 隔离 `14080/14081/14090/14091/24080/24081/24090/24091`。基础与 China 的 10 个链节点逐节点摘要一致；两条链分别为 `14071 -> 14080 -> 14081 -> 14090 -> 14091` 和 `24071 -> 24080 -> 24081 -> 24090 -> 24091`，其中固定客户端 `14071/24071` 的最低等级均为 999，当前服务端也不存在。8 个候选最低等级均为 58，客户端等级为 59/60。固定客户端 `quest_monster.csv` 将 `14080` 固定为 10 种 `LDF4b_T3_Fanatic*` 掉落目标，将 `24080` 固定为其中 9 种，并为 `24081` 指定 `LDF4b_FOBJ_Crack_RiderM/702093`；`quest_script_monster.csv` 为 `14081/24081` 分别指定物品使用区、`233869/233870` 击杀和共同感知区 `LDF4b_SensoryArea_Q14081`。13 个目标 NPC/对象和 5 个任务物品均唯一解析且服务端模板存在，但目标 spawn 全为 0。

真端证据需要分层解释，不能再把原始 DLL 数字直接当成零行为或完整行为。`ScriptDLL64.dll` 的 little-endian 32 位命中为 `14080=952, 14081=29, 14090=2, 14091=2, 24080=0, 24081=34, 24090=2, 24091=2`；其中大量命中来自地址或数据碰撞。排除 `FUN_180c14080` 这类地址后，恢复源码的语义整数命中为 `0/26/2/2/0/31/2/2`。`14081/24081` 在 `fun_620/649/650/867/892/906.cpp` 中有名称绑定、handler 注册、进度、领奖和完成回调；`14090/24090/14091/24091` 只在 `fun_729.cpp` 的 `LDF4b_Alias_Garnon_Entrance` 状态门禁中出现；`14080/24080` 没有恢复到语义 Quest 整数。9 个真端通用行为 XML 对这 8 个 ID 的覆盖均为 0。AC5.8 的 8 个旧社区 handler 全部只有等级/区域任务注册、空 `switch(targetId)`，没有 Quest NPC 注册或真实推进调用，部分仍含自动生成 TODO，因此只作为非权威检索索引，不复制。

固定客户端仍只有 `LDF4b=600030000/116 subzones` 元数据和周边资源，没有 `Levels/LDF4b`、`Level.pak` 或 `PathFind.pak`；AionEmu 同样没有 `600030000` world、GEO、spawn map 或目标 spawn。即使 `14081/24081` 已恢复部分真端回调，也无法在用户指定的“不修改客户端”约束下形成可运行闭包。迁移器 v20 只生成 `d_batch_06_isolation.json`，不生成模板、Java handler 或 XML 行为；AI 状态为 `8/8 isolated`，未来恢复前必须重新审计物品使用区、感知区、动态 NPC/对象生命周期、归属、清理、失败恢复及 Quest 进度/领奖/完成闭包。正式隔离总数由 27 增至 35。旧“生产 D 类剩余 17”重复包含 D batch 03 已隔离的 `18412/28412`，去重后本批前实际未决为 15，本批后剩余 7：`2010/9510/9615/9652/12999/15097/20015`。Quest 覆盖仍为 `6476 / 6462 / 3573 / 81`，不修改客户端。

D batch 07 隔离最后 7 个生产 D 类 `2010/9510/9615/9652/12999/15097/20015`。基础与 China 模板逐节点 SHA-256 完全一致，依次为 `a8133062.../888e557f.../39468398.../c6cb4892.../a207d817.../7c53993c.../77ebd2f0...`；客户端 `quest_monster.csv`、`quest_script_monster.csv` 和 `data_driven_quest.xml` 对 7 个 ID 均为零覆盖，真端 9 个通用 Quest 行为 XML 也全部为零。真端 `quest.xml.dev_name` 分别说明：`2010` 为魔族任务、`9510` 为“Quest 计时器专用且禁止删除”、`9615` 为无限对战隐藏任务、`9652` 为潘盖亚 PVP 脚本任务、`12999` 为 4.0 开场电影、`15097` 为共用舰船视频、`20015` 为 5.5 开场电影隐藏任务；这些开发用途注释不能替代运行行为。

恢复源码的语义整数命中为 `2010/9510/9615/9652/12999/15097/20015 = 9/0/2/0/0/0/0`：`2010` 的 9 次全部只是 Visual Studio 2010 编译器库注释；`9615` 的 `FUN_180ef5550` 在事件参数 Quest ID 为 `0x258f` 时调用未知虚表槽 `+0x3d0`，`FUN_180ef9130` 在该 Quest 状态字节为 `3` 时调用同一槽。完整 Ghidra 分析只找到 `180b923e0/186624e84 -> FUN_180ef5550`、`180b932a0/1866253ac -> FUN_180ef9130` 四个未识别数据/注册块引用，没有恢复接取、推进、完成、失败或清理闭包。原始 DLL 32 位命中为 `1/0/7/0/0/0/0`，不单独作为行为证明。NPCServer `fun_040.cpp` 只证明 `12999` 属于用户仍在加载时允许接收 AddQuest 的特殊 ID，不能证明谁触发、播放哪部电影或如何完成。

社区证据只作为反证索引：`ascension.xml` 将 `2010` 标为 `[UNUSED]`；`sanctum.xml` 虽为 `9510` 提供 `report_to 203752 -> 203711`，但它与真端计时器用途冲突且没有任何真端行为源支持，按非权威实现拒绝采用；`narakkalli.xml` 对 `20015` 只有 `[TODO]`。固定客户端 `CutSceneMovies.xml` 确有电影 `18/mv_LDF4a_Intro_Li.bik`、`19/mv_LDF4a_Intro_Da.bik`、`32/mv_F5_Intro.bik`，但没有 `12999/15097/20015` 到电影 ID 的权威绑定，禁止按文件名猜测。迁移器 v21 因此只生成 `d_batch_07_isolation.json`，7/7 不生成模板、Java handler 或 XML 行为；AI 审计均为 `not_applicable`、生产阻断为 0。正式隔离总数由 35 增至 42，生产 D 类未决由 7 降至 0，Quest 覆盖保持 `6476 / 6462 / 3573 / 81`，不修改客户端。

`80315/80321` 不发布。真端 `Quest_SimpleUseItem.xml` 已证明物品接取、一次 Talk 和交付关系，现有 `report_to_many start_item_id` 足以表达；但必需 NPC `831423/831424/831427/831428` 只有模板，没有服务端 spawn，58Server 静态 `Map/Worlds` 也没有权威坐标。迁移器 v18 将两者固化到 `event_item_isolation.json`，禁止借相邻 `80314/80316/80320/80322` 的行为猜出生位置。
...
| `can_report` | 6 | 6 | 已完成字段映射、通用领奖和第二批行为/入口闭包；当前剩余 82 个一级候选中为 0 |
...
- 一级候选的 42 个 D 类已全部终审：11 个 `DataDriven Empty` 任务维持 E 隔离；D batch 01/02/03/04 已恢复 `18604/28604/30810/18744/28744/11295`，`18412/28412` 因证据无法形成一致配对保持隔离；D batch 05/06 分别隔离 8 个世界绑定任务；D batch 07 将最后 `2010/9510/9615/9652/12999/15097/20015` 7 个缺失闭包任务正式隔离。生产 D 类剩余 0。
...
因此候选范围、39 个疑似测试/开发任务的 E 隔离、名称解析、客户端目标核对、`cannot_share` 同步、`can_report` 通用能力、BM 兼容 no-op、第二至五批、C batch 01/02/03/04 与 D batch 01/02/03/04 Quest 运行闭包，以及 D batch 05/06/07、`18412/28412`、`41600-41614`、`80315/80321` 的隔离终审已经完成。生产 C/D 类开发未决均为 0；当前最大耗时只剩已发布 Quest 的真实运行验收，以及 `50019/51019/80341` 的入口取证。LDF4a/LDF4b 相关任务维持已记录的隔离/静态发布限制，不消耗当前开发阶段时间；`2055` 和其余 3,406 个等级 999 Quest 全部后置。
...
`QuestTemplate.isCannotShare()` 对缺失属性返回 `false`。完成共有 Quest 同步和 D batch 03 发布后，当前 XML 中有 3,955 个 Quest 显式设置 `cannot_share="true"`、766 个显式为 `false`、1,754 个缺失该属性并在服务端模板层默认允许分享；客户端按钮是否点亮还取决于客户端自己的 Quest 数据和 UI 判断。
...
当前双方共有的 6,459 个 Quest 中，迁移前已有范围曾有 1,009 个 `cannot_share` 差异，现已全部同步；全部已发布批次也与固定客户端一致：
...
只有全部门槛通过后，人工审查生成差异，再替换正式资源。`GENERATED_OUTPUTS` 是唯一受管输出允许清单；不在清单中的旧 Quest XML、旧行为 XML 和旧报告在每次生成前删除，不能作为隐式输入继续参与迁移。
...
- 正式发布前按 Quest ID 扫描旧模板、旧 XML 行为和旧 Java handler；被新实现替代的文件必须删除，最终只允许一个有效模板和一条明确行为注册路径。
- 旧实现若包含与固定客户端或真端证据冲突的兼容路径，不能以“备用逻辑”保留；批次报告或发布 manifest 必须记录清理策略并证明清理后残留为 0。
...
当前已在数据转换仓库实现 `/Users/mc/PycharmProjects/aion_drop/scripts/quest_migration_audit.py`。它复用现有 `/Users/mc/PycharmProjects/unpak/aion_pak.py`，把基础、China、world 和 CutScene PAK 解包到系统临时目录，完成摘要校验后自动删除临时文件；正式客户端资源始终只读。后续 Quest 迁移脚本也必须继续放在 `aion_drop`，不得回写到 AionEmu 的 `scripts/`。
...
/Users/mc/PycharmProjects/aion_drop/target/quest-migration/
  ...
  quest_data.d_batch_01.generated.xml
  quest_data.d_batch_02.generated.xml
  quest_data.d_batch_03.generated.xml
...
- 迁移前 `10,035 / 6,424 / 6,410 / 3,625 / 14`、首批后 `10,035 / 6,442 / 6,428 / 3,607 / 14`、第二批后 `10,035 / 6,448 / 6,434 / 3,601 / 14`、第三批后 `10,035 / 6,458 / 6,444 / 3,591 / 14`、第四批后 `10,035 / 6,459 / 6,445 / 3,590 / 14`、第五批后 `10,035 / 6,460 / 6,446 / 3,589 / 14`、C batch 01 后 `10,035 / 6,462 / 6,448 / 3,587 / 14`、C batch 02 后 `10,035 / 6,464 / 6,450 / 3,585 / 14`、C batch 03 后 `10,035 / 6,466 / 6,452 / 3,583 / 14`、C batch 04 后 `10,035 / 6,470 / 6,456 / 3,579 / 14`、D batch 01 后 `10,035 / 6,472 / 6,458 / 3,577 / 14`、D batch 02 后 `10,035 / 6,473 / 6,459 / 3,576 / 14`、D batch 03 后 `10,035 / 6,475 / 6,461 / 3,574 / 14`、D batch 04 后 `10,035 / 6,476 / 6,462 / 3,573 / 14` 十四个基线；
- 迁移前一级候选 133、首批后 115、第二批后 109、第三批后 99、第四批后 98、第五批后 97、C batch 01 后 95、C batch 02 后 93、C batch 03 后 91、C batch 04 后 87、D batch 01 后 85、D batch 02 后 84、D batch 03 后 82、D batch 04 后 81 的基线；
...
- D batch 01 `18604/28604` 固定客户端模板、真端多边形与反编译行为证据、共享 Java handler、NPC/spawn/world/GEO 和尸体恢复 AI 门禁闭包生成；
- D batch 02 `30810` 固定客户端模板、真端 `fun_648/875.cpp` 反编译行为证据、电影 `36`、中断恢复、单 handler 与零旧 XML 行为闭包生成；
- D batch 03 `18744/28744` 固定客户端模板与 `cannot_share=false`、CutScene `912/913`、真端 DLL 与 `fun_006/725.cpp` 证据、共享 Java handler、中断重播、电影结束完成及 `18412/28412` 零残留隔离门禁生成；
- D batch 04 `11295` 固定客户端模板与 `check_item1_1` 物品门禁、CutScene `488`、真端 DLL 注册/回调字节签名、现有副本电影触发、单 Java handler 和零旧 XML 行为闭包生成；
- D batch 05 `10070-10073/20070-20073` 的基础/China 模板一致性、等级 999 前置链、LDF4a/LDF4b 客户端目标与世界元数据、客户端/服务端关卡资产缺失、真端 DLL 常量缺失、正式资源零实现和 AI 隔离报告生成；
- D batch 06 `14080/14081/14090/14091/24080/24081/24090/24091` 的基础/China 链、客户端掉落/脚本目标、13 个 NPC/对象和 5 个物品解析、真端 XML 零覆盖、恢复源码语义命中、DLL 原始碰撞、旧社区空壳、LDF4b 世界缺失、正式资源零实现和 AI 隔离报告生成；
- D batch 07 `2010/9510/9615/9652/12999/15097/20015` 的基础/China 节点摘要、客户端 3 个行为源和真端 9 个通用行为 XML 零覆盖、`fun_874.cpp` 两段 `9615` 未知引擎调用、DLL 原始命中、NPCServer `12999` 加载期特例、社区 UNUSED/冲突/TODO、电影候选无 Quest 绑定、正式资源零实现和 AI 隔离报告生成；
...
- 每次生成前按 v21 固定输出清单删除被替代的旧 Quest XML、Quest 行为 XML 和报告 JSON/CSV，并在 `release_manifest.json` 中固定记录清理策略与“清理后残留为 0”；不触碰 `client-level-audit`、`client-quest-audit`、`spawn-audit` 等历史取证目录；CutScene 证据只在系统临时目录解包，不保留批次级 CutScene 目录；
- 相同输入重复运行得到全部输出摘要完全一致。

生成器不直接覆盖正式资源；生成 XML 经人工审查和验证后发布到 AionEmu。首批报告固定为 18 个模板、13 个生成行为；第二批报告固定为 6 个模板、6 个生成行为、6 条运行闭包；第三批报告固定为 10 个模板、2 个生成行为、10 条运行闭包；第四批和第五批各为 1 个模板、0 个新行为、1 条运行闭包；C batch 01/02/03 各为 2 个模板、2 个现有通用类型行为、2 条运行闭包；C batch 04 为 4 个新模板、4 个具体 Java handler、2 条 XML 击杀行为和 6 条运行闭包；D batch 01 为 2 个模板、1 个共享 Java 基类、2 个薄 handler 和 2 条运行闭包；D batch 02 为 1 个模板、1 个 Java handler、0 条 XML 行为和 1 条运行闭包；D batch 03 为 2 个模板、1 个共享 Java 基类、2 个薄 handler、0 条 XML 行为和 2 条运行闭包，并将 `18412/28412` 的模板/Java/XML 固定为 0；D batch 04 为 1 个模板、1 个 Java handler、0 条 XML 行为和 1 条运行闭包；D batch 05/06 各只固定 8 条隔离记录，D batch 07 只固定 7 条隔离记录，均不生成模板、Java handler 或 XML 行为；所有已发布 D 批次的未解析字段和引用均为 0。`51019` 已按客户端/模板收集语义把现有不兼容的 `monster_hunt` 修正为 `item_collecting`；`80341` handler 已从旧的 `831541-831548` 改为真端行为明确的 `831709`，但这两项仍须通过真实入口运行验收。
...
### 阶段 2：同步双方共有 Quest（迁移前 6,410，当前 6,459）
...
- D batch 01 已发布 `18604/28604`：固定客户端模板优先，真端世界区域和 7 个反编译函数补全服务端行为；共享 handler、尸体恢复 AI 门禁、NPC/spawn/world/GEO 均为 `2/2 closed`。
- D batch 02 已发布 `30810`：固定客户端模板优先，真端 `fun_648/875.cpp` 补全 10 级隐藏接取、电影 `36`、结束完成和进世界中断恢复；单 handler、零旧 XML 行为为 `1/1 closed`。
- D batch 03 已发布 `18744/28744`：固定客户端模板和分享语义优先，CutScene `912/913`、真端 DLL 与 `fun_006/725.cpp` 补全 Raksang 进世界接取、中断重播和电影结束完成；共享 handler、两个薄 handler、零旧 XML 行为为 `2/2 closed`，`18412/28412` 维持零实现隔离。
- D batch 04 已发布 `11295`：固定客户端模板和 `182215009 × 2` 门禁优先，CutScene `488`、真端 DLL 注册/回调字节签名和现有 Padmarashka 副本电影触发形成 `1/1 closed`；单 handler、零旧 XML 行为，不修改客户端和实例脚本。
- D batch 05 已隔离 `10070-10073/20070-20073`：固定客户端等级 999 前置、LDF4a/LDF4b 目标与缺失关卡、真端 DLL 行为常量缺失和服务端正式资源零实现共同形成 `8/8 isolated`；不新增模板或行为，不修改客户端。
- D batch 06 已隔离 `14080/14081/14090/14091/24080/24081/24090/24091`：固定客户端等级 999 前置、LDF4b 目标与缺失关卡优先；真端对 `14081/24081` 的部分多阶段回调已冻结，但不足以越过世界阻断；不新增模板或行为，不修改客户端。
- D batch 07 已隔离 `2010/9510/9615/9652/12999/15097/20015`：客户端模板、真端用途注释、恢复源码/DLL、NPCServer 加载期特例、社区参考和电影资产均已分层冻结；7 个任务都缺少完整运行闭包，不新增模板或行为，不修改客户端。
- 旧“剩余 17”去除已在 D batch 03 隔离的 `18412/28412` 后实际未决为 15；D batch 06 处理 8 个、D batch 07 处理最后 7 个，生产 D 类剩余 0。11 个 `DataDriven Empty` D 类维持 E 隔离。
- 3,406 个客户端最低等级 999 Quest 和全部明确测试任务保持 E 隔离，不进入当前实施排期。
- 不将无行为任务以“不可接取模板”形式混入正式数据，除非项目明确需要仅展示记录。
- 新批次发布前先删除被替代的旧模板、旧行为和旧受管生成物；同一 Quest 最终只保留一个正式模板和一条有效行为注册路径。

状态：已完成生产终审。D batch 01/02/03/04 已完成发布闭包，D batch 05/06/07 已完成隔离终审，生产 D 类剩余 0；验收要求仍是不存在被统计为已迁移但无法完成、或因旧实现残留走错流程的 Quest。
...
- 总 Quest 数、共有/独有/扩展数；
- A/B/C/D/E 数量；
- 一级候选 133、原 E 隔离 39、生产审计池 94、首批 18、第二批 6、第三批 10、第四批 1、第五批 1、C batch 01/02/03 各 2、C batch 04 新模板 4、D batch 01 新模板 2、D batch 02 新模板 1、D batch 03 新模板 2、D batch 04 新模板 1、当前剩余一级候选 81，以及 LDF4b 地图级隔离 15、活动物品任务隔离 2、D batch 03 配对隔离 2、D batch 05/06 世界链各隔离 8、D batch 07 隔离 7、正式隔离合计 42、生产 D 类剩余 0 的基线；
...
| 项目 | 结果 |
|---|---:|
| D batch 04 发布后覆盖 | `10035 / 6476 / 6462 / 3573 / 14` |
| D batch 04 发布后剩余一级候选 | 81 |
...
| D batch 01 模板 / 薄 handler / 运行闭包 | 2 / 2 / 2 |
| D batch 01 共享 handler / 精确多边形 | 1 / 1 |
| D batch 01 未解析字段 / 引用 / AI 阻断 | 0 / 0 / 0 |
| D batch 02 模板 / Java handler / XML 行为 / 运行闭包 | 1 / 1 / 0 / 1 |
| D batch 02 未解析字段 / 引用 / AI 阻断 | 0 / 0 / 0 |
| D batch 03 模板 / 薄 handler / XML 行为 / 运行闭包 | 2 / 2 / 0 / 2 |
| D batch 03 共享 handler / 隔离 Quest / AI 阻断 | 1 / 2 / 0 |
| D batch 04 模板 / Java handler / XML 行为 / 运行闭包 | 1 / 1 / 0 / 1 |
| D batch 04 未解析字段 / 引用 / AI 阻断 | 0 / 0 / 0 |
| D batch 05 隔离 Quest / 正式模板 / Java handler / XML 行为 | 8 / 0 / 0 / 0 |
...
2026-07-19 C batch 04 复核：迁移器 v15 继续保持该批冻结基线；完整审计的 C batch 04 结果仍为 `6470 / 6456 / 3579 / 87`。候选模板、`reshanta.xml` 和 `zones_quest.xml` 均通过对应 XSD；JDK 25 `ReshantaQuestMigrationTest` 为 2/2、`XmlDataLoaderTest` 为 20/20。`1867/2852` 的 5 次 PVP、`1868/2868` 的两条八区域顺序、`1869/2869` 的两组各 6 次击杀、NPC/spawn/world/GEO 全部为 `closed`，运行闭包 `6/6`，AI 生产阻断为 0。

2026-07-19 D batch 01 复核：迁移器 v15 语法检查与自检通过；清理自检证明被替代的旧 Quest 生成 XML、行为 XML、报告 JSON/CSV 会在生成前删除，而固定输出和历史取证目录保留。相同冻结输入复跑的 12 个 Quest XML、6 个行为 XML、29 个报告聚合摘要均为 `b3b957824c58b0cddc5aa0a3b627a9294af9d3bbb25294f7b8a08e47c873eea8`。完整审计结果为 `6472 / 6458 / 3577 / 85`，`18604/28604` 为 `released=true`、`common_diff_rows=0`、运行闭包 `2/2 closed`、未解析字段/引用 `0/0`、AI 阻断 `0`；候选模板和正式 `zones_quest.xml` 通过对应 XSD，`PetrifiedRotanQuestMigrationTest` 为 3/3、`XmlDataLoaderTest` 为 20/20。正式全量 `quest_data.xml` 的 XSD 仍只被本批未触及的两处历史 `exp="581250 "` 尾空格阻断。

2026-07-19 D batch 02 复核：迁移器 v16 语法检查、自检和两次完整审计通过；受管允许清单为 13 个 Quest XML、6 个行为 XML、30 个报告，共 49 个文件，残留旧输出为 0，连续复跑聚合摘要均为 `a227fb2549ea39c0facb23703d8f21d1d6fa55c1187fd06b18985403864970a7`。完整审计结果为 `6473 / 6459 / 3576 / 84`，`30810` 为 `released=true`、`common_diff_rows=0`、运行闭包 `1/1 closed`、未解析字段/引用 `0/0`、AI 阻断 `0`；门禁为 `single_template=true`、全 Quest handler 树 `single_handler=true`、全 Quest XML 树 `no_legacy_xml_behavior=true`。候选模板通过当前 XSD；`UpdateIntroQuestMigrationTest` 为 3/3、`XmlDataLoaderTest` 为 20/20。正式全量 `quest_data.xml` 的 XSD 仍只被本批未触及的第 3396、3399 行两处 `exp="581250 "` 尾空格阻断。

2026-07-19 D batch 03 复核：迁移器 v17 语法检查、自检和连续两次完整审计通过；受管允许清单为 14 个 Quest XML、6 个行为 XML、31 个报告，共 51 个文件，残留旧输出为 0，两次输出聚合摘要均为 `f19501449879eda0699d5d5efe81957b2f1c63fa6a483b73f4d45f6e49b4388c`。完整审计结果为 `6475 / 6461 / 3574 / 82`，`18744/28744` 为 `released=true`、`common_diff_rows=0`、运行闭包 `2/2 closed`、未解析字段/引用 `0/0`、AI 阻断 `0`；固定客户端 CutScene 与真端 DLL/反编译证据全部为真，门禁为每 ID `single_template=true`、`single_handler=true`、`no_legacy_xml_behavior=true`，`18412/28412` 的模板/Java/XML 均为 0。候选模板通过当前 XSD；在无关的实例数据编译线导致 `StaticData.java:346` 缺少 `instanceCooltimeData` 的情况下，使用最后成功构建的应用类路径完成本批窄编译，`RaksangIntroQuestMigrationTest` 为 3/3、`XmlDataLoaderTest` 为 20/20。正式全量 `quest_data.xml` 的 XSD 仍只被本批未触及的第 3396、3399 行两处 `exp="581250 "` 尾空格阻断。

2026-07-19 D batch 04 复核：迁移器 v18 语法检查、自检和连续两次完整审计通过；受管允许清单为 15 个 Quest XML、6 个行为 XML、32 个报告，共 53 个文件，残留旧输出为 0。完整审计结果为 `6476 / 6462 / 3573 / 81`，`11295` 为 `released=true`、`common_diff_rows=0`、运行闭包 `1/1 closed`、未解析字段/引用 `0/0`、AI 阻断 `0`；客户端物品门禁、CutScene `488`、真端 DLL 两条注册和两条回调签名、现有副本电影触发及 world/GEO/spawn/zone/item 资源全部为真，门禁为 `single_template=true`、`single_handler=true`、`no_legacy_xml_behavior=true`。候选模板通过当前 XSD；JDK 25 完整主源码/测试源码编译成功，`PadmarashkaCutsceneQuestMigrationTest` 为 10/10。候选模板、D batch 04 报告和发布清单 SHA-256 分别为 `4dd13ea91f230b7caa9d3134dbbfab16a81ae9a71cf8c455184e445116a0670e`、`68ed83f9440b526118a86822d43cf50b5075c416d3456e9d061ab336ce8ae700`、`e4f96720918510567edb12cb9bb630cee71cef67431987084fc3d7c2ea5a5b08`。

2026-07-19 D batch 05 复核：迁移器 v19 语法检查、自检和连续两次完整审计通过；受管允许清单为 15 个 Quest XML、6 个行为 XML、33 个报告，共 54 个文件，残留旧输出为 0。完整审计结果保持 `6476 / 6462 / 3573 / 81`，`10070-10073/20070-20073` 为 `8/8 isolated`，正式模板、Java handler、XML 行为均为 0，AI 生产阻断为 0。基础/China 节点摘要、两条前置链、`10064/20064` 等级 999、LDF4a/LDF4b 客户端目标、两张世界元数据、客户端 Level/PathFind 缺失、AionEmu world/GEO/spawn 缺失、五个目标 NPC 无 spawn、14 个任务物品模板存在、真端 DLL Quest/物品常量零命中均已冻结；`300510000/300520000` 副本资源存在但明确标记为不能替代 `600020000/600030000` 开放世界。D batch 05 报告和发布清单 SHA-256 分别为 `27cffae9075ec75d77c80db79aafc67f4c38cdaae548b874b30476f240804667`、`4070145c6b002c23d0a099d8e9daef0fdd3f806810e1fc9e20e4a0b486f1e777`，两次复跑一致。
...
- [x] 迁移前一级候选 133、A/B/C/D 为 15/38/38/42、E 隔离 39、生产审计池 94 已冻结；首批后剩余 115，第二批后剩余 109，第三批后剩余 99，第四批后剩余 98，第五批后剩余 97，C batch 01 后剩余 95，C batch 02 后剩余 93，C batch 03 后剩余 91，C batch 04 后剩余 87，D batch 01 后剩余 85，D batch 02 后剩余 84，D batch 03 后剩余 82，D batch 04 后剩余 81。
...
- [x] D batch 01 `18604/28604` 的固定客户端模板、真端区域与反编译行为证据一致；共享 handler、石像完成、尸体恢复门禁、NPC/spawn、世界和 GEO 闭包为 `2/2 closed`，旧无任务发物品路径及重复 XML/Java 行为均不存在。
- [x] D batch 02 `30810` 的固定客户端模板与真端 `fun_648/875.cpp` 行为证据一致；10 级隐藏接取、电影 `36`、电影结束完成和中断后进世界恢复闭包为 `1/1 closed`，正式资源中只有 1 条模板、1 个 Java handler、0 条旧 XML 行为。
- [x] D batch 03 `18744/28744` 的固定客户端模板、可分享语义、CutScene `912/913` 与真端 DLL/`fun_006/725.cpp` 行为证据一致；Raksang 进世界接取、中断重播和电影结束完成闭包为 `2/2 closed`，每个 ID 只有 1 条模板、1 个 Java handler、0 条旧 XML 行为；`18412/28412` 的三类实现均为 0。
- [x] D batch 04 `11295` 的固定客户端模板、`check_item1_1=quest_18411a 2`、CutScene `488`、真端 DLL 注册/回调签名与现有副本电影触发一致；world/race/level/item 接取门禁和电影结束完成闭包为 `1/1 closed`，只有 1 条模板、1 个 Java handler、0 条旧 XML 行为，不修改客户端和实例脚本。
- [x] D batch 05 `10070-10073/20070-20073` 的基础/China 模板、等级 999 前置链、LDF4a/LDF4b 客户端目标、关卡资产缺失和真端 DLL 零行为命中已冻结；8 个 Quest 为 `isolated`，正式模板、Java handler、XML 行为均为 0，未来恢复动态生成前必须重做 AI 生命周期闭包。
- [x] D batch 06 `14080/14081/14090/14091/24080/24081/24090/24091` 的基础/China 链、等级 999 前置、客户端掉落/脚本目标、13 个目标、5 个任务物品、真端 XML/恢复源码/DLL 分层证据、旧社区空壳和 LDF4b 关卡缺失已冻结；8 个 Quest 为 `isolated`，正式模板、Java handler、XML 行为均为 0，`14081/24081` 的部分回调不被误判为可发布闭包。
- [x] D batch 07 `2010/9510/9615/9652/12999/15097/20015` 的基础/China 模板、客户端/真端通用行为零覆盖、`9615` 两个未知引擎调用、`12999` 加载期 AddQuest 特例、社区 UNUSED/冲突/TODO 和电影候选无 Quest 绑定均已冻结；7 个 Quest 为 `isolated`，正式模板、Java handler、XML 行为均为 0，生产 D 类剩余 0。
...
11. 已完成 D batch 01 `18604/28604`：使用固定客户端模板和真端世界/反编译证据闭包，不保留旧无任务发物品路径或重复行为注册。
12. 已完成 D batch 02 `30810`：使用固定客户端模板和真端 `fun_648/875.cpp` 闭包电影流程，只保留一个正式 Java handler，不保留旧模板、旧 XML 行为或重复注册。
13. 已完成 D batch 03 `18744/28744`：使用固定客户端模板/CutScene 与真端 DLL/`fun_006/725.cpp` 闭包 Raksang 电影流程，只保留一个共享基类和两个薄 handler；`18412/28412` 保持模板、Java、XML 全为 0 的隔离状态。
14. 已完成 D batch 04 `11295`：使用固定客户端模板/物品门禁、CutScene `488`、真端 DLL 字节证据和现有 Padmarashka 副本电影触发闭包，只保留一个正式 Java handler，不修改客户端和实例脚本。
15. 已完成 D batch 05 `10070-10073/20070-20073` 隔离终审：固定客户端等级 999 前置和 LDF4a/LDF4b 缺失关卡优先，不复制旧社区 handler，不生成正式资源，不修改客户端。
16. 已完成 D batch 06 `14080/14081/14090/14091/24080/24081/24090/24091` 隔离终审：真端部分回调与原始 DLL 碰撞分层记录，固定客户端 LDF4b 缺失关卡优先，不复制旧社区空壳，不生成正式资源，不修改客户端。
17. 已完成 D batch 07 `2010/9510/9615/9652/12999/15097/20015` 隔离终审：客户端模板、真端用途、反编译/DLL、NPCServer、社区和电影证据分层冻结，不猜计时器、PVP 或电影闭包，不生成正式资源，不修改客户端；生产 D 类剩余 0。
18. 完成已发布 Quest 的真实在线角色运行验收，并继续寻找 `50019/51019/80341` 的不猜值入口证据。
19. 3,406 个最低等级 999 Quest 和 120 个明确测试任务不进入当前排期。

## 21. 最终判定

保持当前 Quest 格式同步真端数据是正确方向，也比引入第二套 Quest 引擎或直接读取真端 221 字段风险更低。

可以较高自动化完成的是 Quest 模板字段、常规奖励、限制、掉落、前置和严格 B 类行为；四个生产 C 批次和 D batch 01/02/03/04 已经闭包，D batch 05/06/07 已按固定客户端和真端证据正式隔离，生产 C/D 类开发未决均为 0。最后 15 个生产 A/B Quest 已按固定客户端关卡资产缺失正式隔离，不再属于待开发承诺。动态宏、名称索引、faction、`cannot_share`、`can_report`、`bm_restrict_category`、`quest_permitted_worlds` 的本批行为门控和满足条件的末次 `reward_repeat_count` 已确认不再阻塞当前发布。
...
当前结论是：技术路线可行，转换器、分阶段覆盖基线、共有 Quest 分享/上报字段同步、自动领奖通用能力、BM 兼容 no-op、首批静态发布、第二批 6 个要塞自动领奖 Quest、第三批 10 个普通生产 Quest、第四批 `2585`、第五批 `21224`、四个 C 批次和 D batch 01/02/03/04 运行闭包、护送和动态生成 AI 审计，以及 D batch 03 配对隔离、D batch 05/06/07、LDF4b/活动物品任务隔离终审均已落地。动态生成 46 条生产动作、`siege_report_hunt` 6 条链、第三批 10 条、第四批 1 条、第五批 1 条、前三个 C 批次各 2 条、C batch 04 的 6 条、D batch 01 的 2 条、D batch 02 的 1 条、D batch 03 的 2 条和 D batch 04 的 1 条运行闭包均已清零阻断；D batch 05/06 均为 `8/8 isolated`，D batch 07 为 `7/7 isolated`，`18412/28412`、`2055`、`41600-41614`、`80315/80321` 与其余等级 999 内容按证据隔离。52 个 Quest 已从候选转为正式模板，发布后剩余一级候选为 81，正式隔离总数为 42，生产 D 类剩余 0。下一步只需完成已发布 Quest 的真实在线运行验收和 `50019/51019/80341` 的入口取证；保护类在具体 handler 和事件分发入口均为 0 的现状下不得猜测实现，3,406 个等级 999 Quest 全部后置。
