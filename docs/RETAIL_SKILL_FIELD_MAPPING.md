...
当前正式运行 compact Bundle 包含 14,517 条技能模板：14,457 条真端模板加 60 条 AionEmu 本地扩展。正式 `groups.xml` 字段字典有 228 个唯一字段，其中 16 个是技能级原始保留字段，212 个是 `effect1`～`effect4` 原始字段。最新 V3 生成 Bundle 的字段字典为 227 个；正式 Bundle 额外保留了已编译的原始 `delay_type`，所以两边 `<retail>` 的数字字段 ID 不可直接互换。完整同步只对齐可执行属性、条件和效果，并保留正式 Bundle 自己的 `<retail>` 审计片段。下文 14,494、14,410 等数量是上一版 V3 字段审计快照，只用于保留当时的对照证据。

NPC 技能表另有 317 个无 ID 源槽，分布于 84 个技能组并影响 156 个 NPC。真端
`fun/fun_064.cpp:202-256` 的 `XML_ParseNPCAttack` 在 `SkillDB_FindSkill` 返回空指针时只记录错误并继续加载；
AionEmu 因此保留原始槽位索引并将该槽运行值设为 `null`，与真端一致。客户端
`data/skills/skills.pak` 仅包含技能图标和快捷栏 XML，不是名称到 ID 的模板来源；禁止据此或按相邻技能猜 ID。
...
| `type` | `SkillTemplate.type` / `skilltype` | 枚举规范化；上一版快照有 508 个共同技能值差异 |
| `sub_type` | `SkillTemplate.subType` / `skillsubtype` | 枚举规范化；上一版快照有 42 个值差异 |
| `skill_group_name` | `SkillTemplate.skill_group_name` | 字符串映射；上一版快照有 11 个值差异 |
| `delay_id` | `SkillTemplate.delayId` | 整数映射；上一版快照在双方均有值时存在 25 个差异 |
| `delay_type` | `SkillTemplate.delayType` | `1` 时冷却乘施法者最终 `AttackDelay / 1000`；`0` 固定，当前 64 条非零记录的 cooldown 均为 0 |
| `activation_attribute` | `SkillTemplate.activationAttribute` | 枚举规范化；上一版快照有 27 个差异 |
| `dispel_category` | `SkillTemplate.dispelCategory` | `Buff/DebuffPhy/DebuffMen/Npc_*` 枚举转换 |
| `required_dispel_level` | `SkillTemplate.reqDispelLevel` | 整数映射；上一版快照有 78 个差异 |
| `target_slot` | `SkillTemplate.targetSlot` | 枚举规范化；上一版快照有 485 个差异 |
| `hostile_type` | `SkillTemplate.hostileType` | 双方均有值的 4,895 个技能完全一致 |
| `cancel_rate` | `SkillTemplate.cancelRate` | 整数映射；上一版快照有 90 个差异 |
| `motion_name` | `Motion.name` | 名称规范化；上一版快照有 79 个差异 |
| `first_target` | `Properties.firstTarget` | 枚举转换；上一版快照有 177 个差异 |
| `first_target_valid_distance` | `Properties.firstTargetRange` | 距离值；上一版快照有 4,639 个差异 |
...
这些历史差异已经由完整 Bundle 同步按真端生成值更新；当前共享模板的可执行 XML 语义复验为 0 差异。
...
| 目标停止 | `target_stop` | `SkillTemplate.targetStop`；客户端 `CM_REMOVE_ALTERED_STATE` 只能结束同技能且允许目标主动停止的效果 |
...
| 连锁技能 | `chain_category_name`、`chain_category_level`、`chain_skill_prob1`、`chain_skill_prob2`、`prechain_category_name`、`prechain_count`、`chain_time`、`self_chain_count` | `chain_skill_prob2` -> `chain_skill_prob`；当前 `chain_skill_prob1` 全部为零，非零值拒绝转换；其余字段生成技能链条件及关联数据 |
...
| 成功惩罚 | `penalty_skill_succ`、`penalty_skill_succ_msg` | `penalty_skill_id` 及施放成功后的动作/消息 |
| 周期消耗 | `cost_time`、`cost_toggle`、`cost_checktime_parameter`、`cost_checktime` | `PeriodicActions` 及 `checktime` |
| 等级时间 | `delay_time_lv` | `ceil(delay_time_lv / 100)` 生成 `cooldown_delta`；运行时按 `cooldown + cooldown_delta × skillLevel` 计算，并限制最低为 0 |
...
这里的目标明确不代表所有时间字段可以统一处理。例如 `casting_delay` 是施法时间，`delay_time` 是基础冷却，`delay_time_lv` 是已经换算为 `cooldown_delta` 的等级增量，`nonchained_delay_time` 则只用于非连锁冷却路径。

## 4. 当前保留的 16 个技能级原始字段

以下清单直接来自当前运行时 `groups.xml`，不再沿用上一版“32 个字段”的 V3 快照。原始字段仍保留不代表完全未编译：转换器可同时生成运行模型并保留尚不能完整表达的原值。

| 字段 | 原始记录数 | 当前结论 |
|---|---:|---|
| `__type_desc__` | 18,233 | 类型说明元数据；当前字段字典只有一个 ID，多次值通过 occurrence 保留 |
| `advancement_rate` | 15 | 全部为 `100`；采集/制作运行路径无消费者 |
| `auto_attack` | 455 | `stop=398`、`maintain=57`；真端 token `0x10b` 直接成功返回，服务器忽略 |
| `charging_delay` | 20 | 写入 Skill `+0xd00`；只参与前置/自身连锁计数路径，当前 20 条均没有这两个计数，数据下不可触发 |
| `delay_type` | 13,039 | `0=12,975`、`1=64`；已编译为运行属性并接入最终攻击间隔冷却缩放 |
| `max_skill_point` | 15 | 采集/制作元数据；运行路径无消费者 |
| `motion_mode` | 93 | 全部为 `Castor_To_Target`；客户端动作表现字段 |
| `penalty_type_succ` | 1 | 仅技能 346，值为 `1`；真端写入 Skill `+0xd4c`，完整引用扫描没有运行读取方 |
| `pre_fx_delay` | 144 | NPCServer 会解析，但尚未找到运行时读取方；继续原样保留 |
| `prechain_skillname` | 62 | 真端 token `0x823` 直接成功返回，服务器忽略 |
| `status_fx_slot` | 2,580 | 状态特效槽位；客户端表现字段 |
| `status_fx_slot_lv` | 135 | 全部为 `50`；客户端表现字段 |
| `status_sfx1` | 4 | 状态音效；客户端表现字段 |
| `system_fire_fx` | 30 | 系统/特效资源字段；未发现服务器战斗消费者 |
| `toggle_id` | 126 | 共有 29 种 ID；不在 MainServer XML token 表中，服务器不解析 |
| `use_arrow_count` | 209 | 全部为 `1`；token `0xce1` 把 Skill 箭矢数清零，`use_arrow` token `0xce0` 也直接返回，当前数据不检查或扣箭 |

原 7 个行为候选字段已经全部闭合：`delay_type` 已进入运行模型；其余字段由真端解析分支、消费者扫描或当前数据组合证明不产生运行行为。它们继续无损保留用于审计，但不再是技能语义发布阻断项。

`delay_type` 的真端解析分支位于 `Server64.exe:0x140b5b3d2`，仅接受 `0/1` 并写入 Skill `+0x208`；
`FUN_140b685b0` 在该值非零时用最终 `AttackDelay`（Creature `+0x14a6`，毫秒）除以 1000 后乘冷却。
AionEmu 在普通施放、蓄力阶段、共享冷却判定和登录冷却动画中统一使用同一缩放函数。

真端仍保留箭矢检查和扣除函数，但它们只有在 Skill `+0x1dc` 非零时才会被选择。当前 `skill_base.xml`
没有 `use_arrow` 记录，209 条 `use_arrow_count=1` 又会被解析分支清零，因此这些函数在当前数据下不可达；
AionEmu 4.0 后也已移除箭矢装备槽，不恢复一套没有真端数据入口的旧弹药机制。

上一版清单中的 `broadcast_use_message`、`damage_attenuation`、`exclusive_attribute`、`exclusive_attribute_tag`、`hide_decrease_count`、`is_familiar_skill`、`max_maintain_count`、`nonchained_delay_time`、`penalty_skill_succ_msg`、`stance2_type`、`target_range_opt3`、`target_stop`、`ultra_skill`、`ultra_transfer` 已不再作为技能级原始字段出现在当前 Bundle，因此不能继续统计为当前遗留。`target_stop`、`damage_attenuation`、`hide_decrease_count`、`ultra_skill` 等已有明确 Java 消费者。
...
| 字段模式 | 槽位 | 当前目标 | 状态 |
|---|---|---|---|
| `effectN_type` | 1～4 | 具体 `EffectTemplate` 子类 | 已对应；231 种拼写归一为 174 种 canonical effect family |
| `effectN_noresist` | 1～4 | `EffectTemplate.noResist` | 已对应 |
| `effectN_remain1`、`effectN_remain2` | 1～4 | `duration1`、`duration2` | 已对应；真端原值和运行时单位均为毫秒 |
| `effectN_effectid` | 1～4 | `EffectTemplate.effectid` | 已对应 |
| `effectN_basiclv` | 1～4 | `EffectTemplate.basicLvl` | 已对应 |
| `effectN_hop_type`、`hop_a`、`hop_b` | 1～4 | `hopType`、`hopA`、`hopB` | 已对应；当前数据只包含 `DAMAGE`、`SKILLLV`，大小写统一后直接生成 |
...
| `effectN_critical_prob_mod1` | 1～3 | `EffectTemplate.critProbMod1` | 已对应；概率为 `mod1 * skillLevel + mod2` |
| `effectN_critical_prob_mod2` | 1～4 | `EffectTemplate.critProbMod2` | 已对应 |
| `effectN_critical_add_dmg_mod2` | 1、3 | `EffectTemplate.critAddDmg2` | 已对应；作为固定暴击附加伤害 |
| `effectN_target_type` | 1～4 | 当前目标效果 | 已闭合；22,125 条大小写变体规范化后全部为 `Target_Only`，无需额外分支 |
| `effectN_checkforchain` | 2～4 | 无运行时输出 | 已闭合；当前 1,934 条全部为 `0` |
| `effectN_checktime` | 1～4 | 周期/延迟检查时间 | 已对应；单位为毫秒，仅由实际读取该字段的效果族生成到 `checktime` |
| `effectN_hidemsg` | 1～4 | 无运行时输出 | 已闭合；四个 token 均直接成功返回，真端不解析值 |
| `effectN_cond_preeffect` 及 `prob1/prob2` | 1～4 | `preEffect`、`preEffectProbDelta`、`preEffectProb` | 已对应；概率为 `prob1 * skillLevel + prob2` |
| `effectN_cond_race` 及 `prob1/prob2` | race 仅槽 2，概率槽 2～3 | 效果族专用种族条件 | 已对应；按 `Shield` 等实际消费者生成，未支持的值拒绝转换 |
| `effectN_cond_status` 及 `prob1/prob2` | 2～4 | 效果 `conditions/subconditions` | 已对应；异常状态、飞行状态按效果族生成，不能表达的概率组合拒绝转换 |
| `effectN_cond_attack_dir` 及 `prob1/prob2` | 2～3 | `conditions/back` 或效果族修正器 | 已对应；当前已证明的背击值和固定概率按效果族生成 |
| `effectN_reserved1`～`reserved22` | 1～4 | 由具体效果类型决定 | 效果专用；共 88 个字段 |
| `effectN_reserved_cond1` 及 `prob1/prob2` | 1～4 | 具体效果的第一组条件 | 已按效果族处理；命中类型等已证明消费者生成运行属性，其余原样保留 |
| `effectN_reserved_cond2` 及 `prob1/prob2` | 1～4 | 具体效果的第二组条件 | 已按效果族处理；种族等已证明消费者生成运行属性，其余原样保留 |

以上 59 种后缀模式覆盖当前字段字典中的 212 个 `effect*` 字段。验收以 `groups.xml` 的字段名集合唯一覆盖为准，不使用上一版 `i=0..356` 索引或容易重复计算的人工分组小计。
...
同理，未被真端消费者读取的专用 `reserved` 槽只保留原值，不推测成新的 Java/XML 运行参数。完整生成要求每个效果实例都能安全输出，但不把“保留原值”误写成不存在的运行语义。

## 6. 首批编译顺序

字段对应和发布的最小实现顺序：

1. 解析并无损保存当前字段字典的 228 个字段，包括同字段的多次 occurrence。
2. 由 `SkillDefinitionLoaderTest` 固定 228 个字段及其中 16 个技能级字段的准确集合。
3. 第 3 节的结构字段按真实技能和运行消费者逐组实现，每组留下至少一个真实技能对照测试。
4. 第 4 节原 7 个行为候选字段已闭合；未编译的元数据/表现字段继续保留原值。
5. 效果层先编译通用元数据，再按 `effectN_type` 分 family 解释 reserved 参数；当前 174 个 family 和 22,427 个实例已全部生成。

当前运行时已经使用完成同步的 compact Bundle；技能级原始字段和效果族均没有发布阻断项。专用 `reserved` 槽仍以真端消费者为边界：有读取方才编译，没有读取方只做无损审计保留。

## 7. 当前 Bundle 审计进度

旧的 `113/145`、`287/319`、`144/174` 等进度使用的是历史批次或上一版 V3 字段全集，与当前 Bundle 的字段字典不是同一统计口径，现已废止。当前可复验基线为 14,517 个正式运行技能模板、228 个原始保留字段（16 个技能级字段和 212 个 effect 字段）；原始保留只用于无损审计，不等于存在未接入的运行语义。

最近一次 V3 输出已全量重建，包含 14,457 个技能模板、174/174 种 canonical effect 和 22,427/22,427 条效果实例，跳过数、未编译效果族和 release blocker 均为 0，XSD 验证通过。

第一轮完整 Bundle 同步更新了 1,115 个正式模板，其中包含 803 个 `properties` 片段和 387 个 `effects` 片段，共涉及 27 个技能分片。随后按真端范围函数复审并修正 421 个 `FireStorm` 和 96 个 `LightningBolt` 的 `properties`。同步保留 60 个目标端独有技能及目标端 `<retail>` 审计片段；最终再次按展开后的 XML 语义比较，14,457 个共享模板的可执行差异为 0。以下逐族段落是证明过程记录，其中“本批继续不重建 V3”等表述只描述当时批次，不代表当前发布状态。

范围类型按真端 `Skill.cpp` 的槽位语义生成：`Fireball` 使用 `opt1=范围、opt3=高度`，`FireStorm` 使用 `opt1=距离、opt2=角度、opt3=高度`，`LightningBolt` 使用 `opt1=宽度、opt2=距离、opt3=高度`。`opt4=back` 继续生成反向判定；这些值不能按一种通用位置映射。
...
- AionEmu 现有 `SimpleRootEffect` 继续使用独立 `KNOCKBACK` 状态和短距离强制后退；抗性改用真端索引 24 对应的 `SIMPLE_ROOT_RESISTANCE`，不再误借 `STAGGER_RESISTANCE`；
...
- 该记录设置 `noresist=1`，因此绕过数值抗性概率检定；`reserved11=100` 继续保留在 `retail_fields`，不重复编译为运行参数；输出与真端按技能 ID 和效果槽位核对后，缺失、额外和字段差异均为 0。
...
- `MoveBehindATK reserved5=-300` 和三条 `BackDashATK reserved13=20` 在对应真端参数检查、位移和伤害路径中均未读取，继续保留在 `retail_fields`；三族共 103 条完成聚焦转换和逐条内存核对。该历史批次当时达到 144/174，现已由本节开头的 174/174 发布基线取代。
