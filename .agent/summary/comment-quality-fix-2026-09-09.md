...
基准：`.agent/rules/i18n.md`（注释须中英双语同义）、`docs/aion-game-terms-en-zh.md`（术语对照）。
...
工具与进度：`.agent/summary/comment_i18n/`（`sm_i18n_audit.py` 按行检测中文段残留英文、
...
工具：`.agent/summary/comment_i18n/tail_coverage.py`、`tail_bilingual_apply.py`、`tail_special_progress.json`。
