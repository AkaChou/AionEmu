...
2. 当前仓库以 `rtk git rev-parse --show-toplevel` 得到的 checkout 根目录为准，不依赖机器绝对路径。
3. 不要混用不同 checkout 的 `target`、运行进程、配置或日志。
4. 开始、测试和交付前检查 `rtk git status --short`，保留用户已有的 dirty 改动。

## 命令与编辑

1. 所有 shell 命令使用 `rtk` 前缀。
...
3. 交付前运行 `rtk git diff --check`，检查状态和 diff 文件清单。
4. 需要全量证明时，确认没有其他 Maven/Javac writer，再串行执行 `rtk mvn clean verify`。
...
1. 常用构建入口为 `rtk mvn test`、`rtk ./package.sh`。
